...
from ..models import AdapterExecutionPlan, RuntimeArtifactMount, ServiceClass, StageInvocation
...
_RECIPE_MODEL_PATHS = {
    "alphafold3": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/alphafold3.py",),
    "bindcraft": (
    ...
    "esmfold2": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/esmfold2.py",),
    "esmfold2-fast": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/esmfold2_fast.py",),
    "freebindcraft": (
        "components/control-plane/src/fs2_serve/scientific_batch/adapters/freebindcraft.py",
        "components/control-plane/src/fs2_serve/scientific_batch/adapters/simple_candidates.py",
    ),
    "mosaic": (
        "components/control-plane/src/fs2_serve/scientific_batch/adapters/mosaic.py",
        "components/control-plane/src/fs2_serve/scientific_batch/adapters/simple_candidates.py",
    ),
    "openfold3": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/openfold3.py",),
    "proteinmpnn": (
        "components/control-plane/src/fs2_serve/scientific_batch/adapters/proteinmpnn.py",
        "components/control-plane/src/fs2_serve/scientific_batch/adapters/simple_candidates.py",
    ),
    "protenix-v2": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/protenix_v2.py",),
    "rfdiffusion": (
...
def assert_profile_identity(
    ...
    *,
    ...
    interface = strict_object(
        ...
        optional=frozenset({"parameter_schema_definition", "parameter_schema_sha256"}),
...
def assert_artifact_requirement(
    ...
    *,
    ...
    required_file_size_bytes: int | None = None,
) -> Mapping[str, object]:
    ...
    if required_file_sha256 is not None or required_file_size_bytes is not None:
        if required_file is None:
            raise ScientificAdapterError("an exact artifact file identity requires required_file")
        manifest = requirement.get("file_manifest")
        if not isinstance(manifest, list):
            raise ScientificAdapterError(f"catalog profile {artifact_id} has no exact file manifest")
        entries = [
            item
            for item in manifest
            if isinstance(item, Mapping) and item.get("path") == required_file
        ]
        if len(entries) != 1:
            raise ScientificAdapterError(f"catalog profile {artifact_id} file identity is ambiguous")
        entry = entries[0]
        if required_file_sha256 is not None and entry.get("sha256") != required_file_sha256:
            raise ScientificAdapterError(f"catalog profile {artifact_id} file digest does not match the adapter")
        if required_file_size_bytes is not None and entry.get("size_bytes") != required_file_size_bytes:
            raise ScientificAdapterError(f"catalog profile {artifact_id} file size does not match the adapter")
    return requirement
...
def standard_runtime_mounts(
    profile: Mapping[str, object],
    artifact_ids: tuple[str, ...],
) -> tuple[RuntimeArtifactMount, ...]:
    """Bind reviewed public artifacts to immutable, pre-owned image roots."""

    mounts: list[RuntimeArtifactMount] = []
    for artifact_id in artifact_ids:
        ...
        content_digest = requirement.get("content_digest_sha256")
        if content_digest is not None and (
            not isinstance(content_digest, str) or _SHA256.fullmatch(content_digest) is None
        ):
            raise ScientificAdapterError(f"catalog profile {artifact_id} content digest is invalid")
        mounts.append(
            RuntimeArtifactMount(
                artifact_id,
                model_root(artifact_id),
    ...
    *,
    ...
    for name, semantic_type, path, sanitize_csv in entries:
        ...
        media_type = (
            ...
            if semantic_type.endswith("json/v1") or resolved.suffix.lower() == ".json"
