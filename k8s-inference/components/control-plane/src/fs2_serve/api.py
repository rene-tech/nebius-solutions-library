...
def create_app(runtime: AppRuntime) -> FastAPI:
    ...
    if runtime.scientific_admin is not None:
        ...
        @app.get(
            ...
            response_model=AdminEnvelope[ScientificCapabilities],
            responses=admin_problem_responses,
        )
        async def admin_scientific_capabilities(
            identity: Annotated[OperatorPrincipal, Depends(operator)],
            params: Annotated[AdminContextParameters, Depends(_admin_context_parameters)],
        ) -> AdminEnvelope[ScientificCapabilities]:
            await admin_access.authorize(
                identity,
                OperatorRole.VIEWER,
                action="scientific_capability.read",
                tenant_id=identity.tenant_id,
            )
            return access_envelope(
                scientific_admin.capabilities(),
                params,
            )

        # Run and artifact reporting exists only when a durable reader is
        # configured. Leaving the routes unregistered keeps the advertised
        # surface equal to what this build can actually answer.
        if scientific_admin.runs is not None:
