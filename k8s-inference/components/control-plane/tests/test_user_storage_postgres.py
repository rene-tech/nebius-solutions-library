...
async def test_durable_encrypted_credentials_and_restart(storage_database):
    ...
    assert await store.pool.fetchval("SELECT count(*) FROM fs2_storage_buckets") == 1
    ...
    with pytest.raises(ConflictError):
        await service.configure("customer-a", StoragePolicy(mode="user"))
...
async def test_runtime_role_can_read_and_write_storage_tables(storage_database):
    async with storage_database.pool.acquire() as conn:
        async with conn.transaction():
            ...
            assert (
                await conn.fetchval("SELECT mode FROM fs2_storage_policies WHERE tenant_id='new-customer'") == "tenant"
            )
