...
from fs2_serve.scientific_batch.codec import (
    LEGACY_STATE_SCHEMA_V6,
    LEGACY_STATE_SCHEMA_V7,
    MAX_STATE_BYTES,
    STATE_SCHEMA,
    state_from_value,
    state_to_value,
...
class FakeExecutionBinding:
    def scheduling_targets(self):
        return {("protein-design", "design"): ("fs2-models", "scientific", None)}
...
async def test_submit_freezes_public_profile_and_never_enters_generic_worker(cipher, hasher) -> None:
    ...
    assert cluster.apply_history[0].namespace == "fs2-models"
    assert cluster.apply_history[0].scheduling == state.scheduling.stages[0]


@pytest.mark.asyncio
async def test_deployment_academic_metadata_never_requires_an_input_receipt(cipher, hasher) -> None:
    store = MemoryStore(cipher, hasher)
    identity = await principal(store)
    repository = FakeScientificBatchRepository()
    controller = ScientificBatchController(
    ...
    deployment_profile["access"] = {
        "profile": "academic",
        "state": "verified",
        "receipt_digest": None,
        "license_gate_scope": "production-promotion-only",
    }
    base_catalog = profile_catalog()
    academic_catalog = ScientificProfileCatalog(
        profiles={"protein-design": ScientificWorkloadProfile(MappingProxyType(deployment_profile))},
        validators=base_catalog._validators,  # type: ignore[attr-defined]
    )

    class AcademicExecutionBinding(FakeExecutionBinding):
        def scheduling_targets(self):
            return {("protein-design", "design"): ("fs2-academic-poc", "academic-scientific", None)}

    class AcademicPlanFactory(FakePlanFactory):
        def plan(self, *args, **kwargs) -> AdapterExecutionPlan:
            execution = super().plan(*args, **kwargs)
            return replace(
                execution,
                invocations=(
                    replace(
    ...
    service = ScientificBatchService(
        ...
        repository=repository,  # type: ignore[arg-type]
        controller=controller,
        profiles=academic_catalog,
        scheduling=scheduling(),
        artifacts=FakeArtifactAccess(pointer),
        execution_binding=AcademicExecutionBinding(),
        plan_factory=AcademicPlanFactory(),
    )
    submitted = await service.submit(
        ...
        idempotency_key="academic-deployment-no-request-receipt",
    )
    state = repository.records[UUID(submitted["operation"]["id"])]
    assert state.access_context.profile == "public"
    assert state.access_context.receipt_digest is None
    await controller.reconcile_once()
    status = await service.status(state.operation_id, principal=identity)
    stage = status["batch"]["stages"][0]
    attempt = stage["attempts"][0]
    assert stage["execution_namespace"] == "fs2-academic-poc"
    assert stage["resolved_local_queue"] == "academic-scientific"
    assert attempt["workload_namespace"] == "fs2-academic-poc"
    assert attempt["workload_name"]
...
async def test_public_http_and_mcp_share_scientific_operation_lifecycle(registry, cipher, hasher) -> None:
    ...
    async with app.router.lifespan_context(app):
        async with httpx.AsyncClient(
            ...
            stage_status = status.json()["batch"]["stages"][0]
            attempt_status = stage_status["attempts"][0]
            assert stage_status["execution_namespace"] == "fs2-models"
            assert stage_status["resolved_local_queue"] == "scientific"
            assert attempt_status["workload_namespace"] == "fs2-models"
            assert attempt_status["workload_name"]
...
def _execution_state_for_codec(*, namespace: str, local_queue_name: str) -> ScientificBatchState:
    plan = ScientificBatchPlan((ScientificStagePlan(stage_id="design"),))
    invocation = StageInvocation(
        stage_id="design",
        shard_id="main",
        argv=("protein-design", "run"),
        environment=(),
        working_directory="/mnt/fs2-scientific/work/design/main",
        consumes=(),
        produces="design-result",
    ...
    return ScientificBatchState.admit(
        ...
        model_id="protein-design",
        variant_id="protein-design-h100",
        input_artifact_id=manifest_artifact_id,
        plan=plan,
        scheduling=snapshot,
...
def test_state_codec_reopens_v6_and_upgrades_on_next_write() -> None:
    value = deepcopy(state_to_value(_execution_state_for_codec(namespace="fs2-models", local_queue_name="scientific")))
    value["schema_version"] = LEGACY_STATE_SCHEMA_V6
    for artifact in value["runtime_artifacts"]:
        artifact.pop("aggregate_tree")
    for invocation in value["adapter_execution"]["invocations"]:
        invocation.pop("namespace")
        invocation.pop("local_queue_name")
    for stage in value["scheduling"]["stages"]:
        stage.pop("execution_namespace")

    reopened = state_from_value(json.dumps(value))
    assert reopened.execution_plan is not None
    assert reopened.execution_plan.invocations[0].namespace is None
    assert reopened.execution_plan.invocations[0].local_queue_name is None
    assert reopened.scheduling.stage("design").execution_namespace == "fs2-models"
    upgraded = state_to_value(reopened)
    assert upgraded["schema_version"] == STATE_SCHEMA
    assert upgraded["adapter_execution"]["invocations"][0]["namespace"] is None
    assert state_from_value(upgraded) == reopened
    mixed_version = deepcopy(value)
    mixed_version["adapter_execution"]["invocations"][0]["namespace"] = None
    with pytest.raises(ValueError, match="invocation fields differ"):
        state_from_value(mixed_version)
    with pytest.raises(ValueError, match="exceeds the durable bound"):
        state_from_value(" " * (MAX_STATE_BYTES + 1))


def test_state_codec_reopens_v7_academic_target_and_upgrades_on_next_write() -> None:
    value = deepcopy(
        state_to_value(
            _execution_state_for_codec(
                namespace="fs2-academic-poc",
                local_queue_name="academic-scientific",
            )
        )
    )
    value["schema_version"] = LEGACY_STATE_SCHEMA_V7
    for artifact in value["runtime_artifacts"]:
        artifact.pop("aggregate_tree")
    for stage in value["scheduling"]["stages"]:
        stage.pop("execution_namespace")
    ...
    reopened = state_from_value(json.dumps(value))
    assert reopened.execution_plan is not None
    invocation = reopened.execution_plan.invocations[0]
    assert (invocation.namespace, invocation.local_queue_name) == (
        "fs2-academic-poc",
        "academic-scientific",
    )
    assert reopened.scheduling.stage("design").execution_namespace == "fs2-academic-poc"
    upgraded = state_to_value(reopened)
    assert upgraded["schema_version"] == STATE_SCHEMA
    assert upgraded["scheduling"]["stages"][0]["execution_namespace"] == "fs2-academic-poc"
    assert state_from_value(upgraded) == reopened
    missing_binding = deepcopy(value)
    missing_binding["adapter_execution"]["invocations"][0].pop("local_queue_name")
    with pytest.raises(ValueError, match="invocation fields differ"):
        state_from_value(missing_binding)
...
def test_scientific_service_refuses_a_missing_immutable_execution_route(cipher, hasher) -> None:
    contract = deepcopy(scheduling().contract)
    del contract["local_queue_routes"]["scientific"]
    repository = FakeScientificBatchRepository()
    pointer = {
        "artifact_id": str(uuid4()),
        "sha256": "1" * 64,
    ...
    with pytest.raises(SchedulingContractError, match="Kueue LocalQueue route is not an object"):
        ScientificBatchService(
            ...
            profiles=profile_catalog(),
            scheduling=SchedulingContractResolver(contract),
            artifacts=FakeArtifactAccess(pointer),
            execution_binding=FakeExecutionBinding(),
        )


def test_scheduling_resolver_uses_only_trusted_stage_namespace_queue_pair() -> None:
    batch_plan = ScientificBatchPlan((ScientificStagePlan(stage_id="design"),))
    default_snapshot = scheduling().freeze(
        service_class="customer-batch",
        model_id="protein-design",
        tenant_id="tenant-a",
        profile=profile_value(),
    ...
    assert default_snapshot.stage("design").execution_namespace == "fs2-models"
    assert default_snapshot.stage("design").resolved_local_queue == "scientific"
    invocation = StageInvocation(
        stage_id="design",
        shard_id="main",
        argv=("protein-design", "run"),
    ...
    assert snapshot.stage("design").execution_namespace == "fs2-academic-poc"
    ...
    with pytest.raises(SchedulingContractError, match="namespace differs"):
        scheduling().freeze_for_execution(
            service_class="customer-batch",
            model_id="protein-design",
            tenant_id="tenant-a",
            profile=profile_value(),
            execution=replace(
                execution,
                invocations=(replace(invocation, namespace="tenant-selected"),),
            ),
        )
...
@pytest.mark.asyncio
async def test_academic_jobset_uses_frozen_namespace_and_queue_metadata(tmp_path: Path) -> None:
    plan = ScientificBatchPlan(
        (
            ScientificStagePlan(
                stage_id="design",
    ...
    invocation = StageInvocation(
        ...
        argv=("protein-design", "run"),
        environment=(),
        working_directory="/mnt/fs2-scientific/work/design/gang",
        consumes=(),
        produces="design-result",
        collector_id="protein-design-output-v1",
        validator_id="protein-design-v1",
    ...
    client = httpx.AsyncClient(base_url="https://kubernetes.test")
    cluster = HttpScientificBatchCluster(
        ...
        controller_id="controller-a",
        fence=Fence(),
        renderer=JobSetRenderer(),
        writes_enabled=False,
        client=client,
    )
    manifest = cluster._prepare(resource, 7)
    labels = manifest["metadata"]["labels"]
    pod_labels = manifest["spec"]["replicatedJobs"][0]["template"]["spec"]["template"]["metadata"]["labels"]
    assert manifest["metadata"]["namespace"] == "fs2-academic-poc"
    assert labels["kueue.x-k8s.io/queue-name"] == "academic-scientific"
    assert pod_labels["kueue.x-k8s.io/queue-name"] == "academic-scientific"
    assert HttpScientificBatchCluster._paths(resource.ref)[0].endswith("/namespaces/fs2-academic-poc/jobsets")
    await client.aclose()
...
def test_execution_map_renders_only_digest_qualified_direct_argv(tmp_path: Path, monkeypatch) -> None:
    ...
    assert bound_plan == replace(
        adapter_plan,
        invocations=(replace(invocation, namespace="fs2-models", local_queue_name="scientific"),),
    )
    snapshot = scheduling().freeze(
        service_class="customer-batch",
        model_id="protein-design",
    ...
    resource = WorkloadResource(
        ...
        kind=WorkloadKind.JOB,
        scheduling=snapshot.stages[0],
        invocation=bound_plan.invocations[0],
        access_context=ArtifactAccessContext(profile="public", receipt_digest=None, tenant_id="tenant-a"),
    )
    ...
    gang = replace(
        ...
        kind=WorkloadKind.JOB_SET,
        gang_size=2,
        invocation=replace(bound_plan.invocations[0], shard_id="gang"),
    )
