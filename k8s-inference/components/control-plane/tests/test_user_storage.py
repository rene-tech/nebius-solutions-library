...
class Repository:
    ...
    async def request_enabled(self, tenant, principal, enabled):
        ...
        credential["requested_action"] = None if enabled else "disable"

    async def complete_action(self, tenant, principal, *, enabled, revoked=False):
        credential = self.credentials[tenant, principal]
        credential.update(
            enabled=enabled,
    ...
    async def replace_credential(self, tenant, principal, value):
        credential = self.credentials[tenant, principal]
        credential.update(
            **value,
            enabled=True,
    ...
    async def view(self, tenant, principal, policy):
        ...
        if credential:
            ...
            return UserStorage(
                state="revoked" if credential["revoked_at"] else "ready" if credential["enabled"] else "disabled",
                mode=policy.mode,
...
class Provider:
    ...
    async def rotate_credentials(self, tenant, principal, group, previous):
        del group
        self.key_calls.append((tenant, principal, "rotation"))
        return dict(
            service_account_id=previous["service_account_id"],
            access_key_resource_id=f"rotated-key-{tenant}-{principal}",
...
async def test_credentials_require_explicit_scope_and_commit_redacted_audit(env):
    ...
    call = audit.append_audit_event.call_args.kwargs
    assert call["action"] == "storage.credentials.disclose"
    assert call["outcome"] == "succeeded"
    assert "sensitive-fixture" not in repr(call)
