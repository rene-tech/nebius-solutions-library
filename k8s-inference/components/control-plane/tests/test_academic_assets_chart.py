...
def test_chart_does_not_publish_a_static_academic_execution_job() -> None:
    """The controller owns per-attempt Jobs; the accepted template remains inert data."""

    documents = render(*BINDING_FLAGS)
    contract = execution_config_map(documents)
    assert contract is not None
    assert "kind: Job" in contract["data"]["job_template"]
    assert not any(
