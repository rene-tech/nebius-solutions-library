...
def promoted_catalog(tmp_path: Path) -> tuple[ScientificProfileCatalog, dict[str, object]]:
    ...
    document["profiles"] = [
        profile for profile in document["profiles"] if "variant_id" in profile
    ]
...
def test_generated_installed_config_map_is_the_production_reader_contract(tmp_path: Path) -> None:
    ...
    assert next(mount for mount in af3_data.mounts if mount.kind == "operator-host-path").sub_path.endswith(AF3_CONTENT)
    ...
    af3_cache = next(mount for mount in af3_inference.mounts if mount.kind == "cache")
    assert af3_cache.claim_name == "scientific-alphafold3-cache"
    assert af3_cache.cache_identity is not None
    assert af3_cache.cache_identity.accelerator_sm == "sm90"
    assert af3_cache.cache_identity.run_as_user == 10001
    assert af3_cache.sub_path == af3_cache.cache_identity.sub_path
    openfold_cache = next(
        mount for mount in renderer.executions[("openfold3", "inference")].mounts if mount.kind == "cache"
    )
    assert openfold_cache.claim_name == "scientific-openfold3-cache"
    assert openfold_cache.sub_path is not None and openfold_cache.sub_path.endswith("/sm90")
    protenix_cache = next(
        mount for mount in renderer.executions[("protenix-v2", "sample-structure")].mounts if mount.kind == "cache"
    )
    assert protenix_cache.claim_name == "scientific-protenix-cache"
    assert protenix_cache.sub_path is not None and f"/{5:064x}/" in protenix_cache.sub_path
...
def test_generator_rejects_wrong_af3_tree_identity(tmp_path: Path) -> None:
    ...
    tree = next(item for item in af3["runtime_artifacts"] if item["artifact_id"] == "alphafold3-public-databases-v3.0")[
        "aggregate_tree"
    ]
...
def test_generator_rejects_untrusted_cache_ownership_or_sm_policy(tmp_path: Path) -> None:
    profiles, profile_document = promoted_catalog(tmp_path)
    localizations = localization_contract(profile_document)
    for field, replacement in (("run_as_user", 0), ("accelerator_sm", "sm80")):
        targets = json.loads(TARGETS.read_text())
        binding = next(
            item
            for item in targets["bindings"]
            if item["model_id"] == "protenix-v2" and item["stage_id"] == "sample-structure"
        )
        binding["compiler_cache"][field] = replacement
        with pytest.raises(ScientificExecutionMapError, match="compiler-cache identity is invalid"):
            build_execution_map(profiles=profiles, targets=targets, localizations=localizations)
