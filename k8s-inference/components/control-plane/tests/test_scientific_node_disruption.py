...
def disrupted(reason="DeletionByTaintManager", status="True"):
    return {"phase": "Pending", "conditions": [{"type": "DisruptionTarget", "reason": reason,
            "status": status, "message": "Taint manager: deleting due to NoExecute taint"}]}
...
@pytest.mark.parametrize("reason,status", [
    ("DeletionByTaintManager", "False"), ("DeletionByTaintManager", "Unknown"),
    ("EvictionByEvictionAPI", "True"), ("GenericEviction", "True"), ("", "True")])
def test_generic_or_inactive_disruption_does_not_make_application_error_retryable(reason, status):
    assert _reported_failure(["workload_failed"], [disrupted(reason, status)], model_id="esmfold2")[1] is FailureKind.APPLICATION
...
def test_model_exception_is_not_retried_due_to_later_disruption():
    pod = disrupted()
    pod["containerStatuses"] = [{"name": STAGE_CONTAINER_NAME,
...
async def test_real_observer_classifies_failed_job_with_pending_evicted_pod(tmp_path, monkeypatch):
    ...
    cluster, client = production._collection_cluster(tmp_path, ref, attempt, pod=pod,
