...
async def test_actual_maintenance_role_purges_expired_captures_by_ttl(debug_database):
    """SAI-01: the maintenance credential deletes old captures by timestamp only."""
    db = debug_database
    store = PostgresDebugStore(db.pool, db.cipher)
    old = row(started_at=NOW - timedelta(days=5))
    recent = row(started_at=NOW)
    for item in (old, recent):
        await store.record(item)

    suffix = uuid4().hex[:10]
    maintenance = f"fs2_debug_maintenance_{suffix}"
    await PostgresStore.migrate_database(
        os.environ["FS2_TEST_DATABASE_URL"],
        Path(__file__).parents[1] / "migrations",
        f"fs2_debug_reporting_{suffix}",
        f"fs2_debug_runtime_{suffix}",
        maintenance,
        f"fs2_debug_activation_{suffix}",
    )

    async def assume(connection):
        await connection.execute(f'SET ROLE "{maintenance}"')  # noqa: S608 - locally generated test role

    pool = await asyncpg.create_pool(os.environ["FS2_TEST_DATABASE_URL"], min_size=1, max_size=1, init=assume)
    try:
        # Cipher is not required for the purge path.
        removed = await PostgresDebugStore(pool).purge_expired(before=NOW - timedelta(days=1))
        assert removed == 1
        async with pool.acquire() as connection:
            # The maintenance role can bound retention but cannot read payloads.
            with pytest.raises(asyncpg.InsufficientPrivilegeError):
                await connection.fetch("SELECT ciphertext FROM fs2_request_debug")
    finally:
        await pool.close()

    assert await store.get(old.id, "tenant-a") is None
    assert (await store.get(recent.id, "tenant-a")) is not None
