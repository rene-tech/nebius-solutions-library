...
def cache_binding(
    *,
    ...
    for requirement in requirements:
        files = requirement["file_manifest"]
        ...
        artifacts.append(
            {
                ...
                "content_sha256": requirement["content_digest_sha256"],
    ...
    return identity.sub_path, identity.marker()


def assert_cache_preparer(pod: dict[str, object], *, mount_name: str, identity: dict[str, object]) -> None:
    init_containers = pod["initContainers"]
    assert isinstance(init_containers, list)
    prepare = next(item for item in init_containers if item["name"] == "prepare-compiler-cache")
    assert prepare["volumeMounts"] == [
        {
            "name": mount_name,
            "mountPath": "/mnt/fs2-compiler-cache",
            "readOnly": False,
        }
    ]
    ...
    assert command[:2] == ["fs2-serve", "scientific-prepare-compiler-cache"]
    expected = {
        "--cache-sub-path": identity["sub_path"],
        "--cache-runtime-image-digest": identity["runtime_image_digest"],
        "--cache-artifact-set-sha256": identity["artifact_set_sha256"],
        "--cache-accelerator-sm": "sm90",
        "--cache-identity-sha256": identity["identity_sha256"],
        "--cache-run-as-user": "10001",
        "--cache-run-as-group": "10001",
    }
...
def af3_execution_value(profile_value: dict[str, object]) -> dict[str, object]:
    ...
    cache_sub_path, cache_identity = cache_binding(
        model_id="alphafold3",
        stage_id="inference",
        variant_id="upstream-v3-0-4",
        requirements=[requirements["alphafold3-parameters"]],
    )
    return {
        "schema": "fs2-serve.nebius.ai/scientific-execution-map/v3",
        "controller_service_account": {
...
def test_af3_two_digest_execution_target_and_physical_mounts(tmp_path: Path) -> None:
    ...
    af3_cache_identity = next(
        mount.cache_identity
        for mount in renderer.executions[("alphafold3", "inference")].mounts
        if mount.kind == "cache"
    )
    assert af3_cache_identity is not None
    assert_cache_preparer(
        inference_pod,
        mount_name="alphafold3-warm-cache",
        identity=af3_cache_identity.marker(),
    )
    ...
    cache_mount = next(
        item for item in inference_pod["containers"][0]["volumeMounts"] if item["name"] == "alphafold3-warm-cache"
    )
    assert cache_mount == {
        "name": "alphafold3-warm-cache",
        "mountPath": "/cache/alphafold3",
        "subPath": next(
            mount for mount in renderer.executions[("alphafold3", "inference")].mounts if mount.kind == "cache"
        ).sub_path,
        "readOnly": False,
    }
    cache_volume = next(item for item in inference_pod["volumes"] if item["name"] == "alphafold3-warm-cache")
    assert cache_volume["persistentVolumeClaim"] == {
        "claimName": "scientific-alphafold3-cache",
        "readOnly": False,
    }
    inference_environment = {item["name"]: item["value"] for item in inference_pod["containers"][0]["env"]}
    assert {
...
def test_af3_operator_storage_cannot_cross_namespace_or_change_host_path(tmp_path: Path) -> None:
    ...
    def wrong_cache_identity(value: dict[str, object]) -> None:
        mount = value["models"][0]["stages"][1]["mounts"][2]  # type: ignore[index]
        changed = replace(
            CompilerCacheIdentity.from_mapping(mount["cache_identity"]),  # type: ignore[index]
            accelerator_sm="sm80",
        )
        mount["cache_identity"] = changed.marker()  # type: ignore[index]
        mount["sub_path"] = changed.sub_path  # type: ignore[index]

    with pytest.raises(ScientificExecutionMapError, match="differs from image/artifacts/SM"):
        af3_renderer(tmp_path, mutate=wrong_cache_identity)

    def unsafe_cache_sub_path(value: dict[str, object]) -> None:
        value["models"][0]["stages"][1]["mounts"][2]["sub_path"] = "../tenant-cache"  # type: ignore[index]

    with pytest.raises(ScientificExecutionMapError, match="subPath is unsafe"):
        af3_renderer(tmp_path, mutate=unsafe_cache_sub_path)
...
def test_openfold3_gpu_stage_gets_exact_deployment_owned_warm_cache(tmp_path: Path) -> None:
    ...
    cache_sub_path, cache_identity = cache_binding(
        ...
        requirements=[requirements["openfold3-openbind-0"], requirements["openfold3-components-bcif"]],
    )
    cache_mount.update({"sub_path": cache_sub_path, "cache_identity": cache_identity})
    stage_common = {
        "execution_namespace": "fs2-models",
    ...
    request = json.loads((REPO_ROOT / "models/structure/runtime/openfold3/fixtures/positive-monomer.json").read_text())
    model_input = ScientificInputArtifact(
        logical_artifact_id="model-input",
        semantic_type="request/v1",
        artifact_id=uuid4(),
    ...
    manifest = renderer.render(resource)
    pod = manifest["spec"]["template"]["spec"]  # type: ignore[index]
    assert_cache_preparer(pod, mount_name="openfold3-warm-cache", identity=cache_identity)
    rendered_mount = next(
        item for item in pod["containers"][0]["volumeMounts"] if item["name"] == "openfold3-warm-cache"
    )
    ...
    rendered_volume = next(item for item in pod["volumes"] if item["name"] == "openfold3-warm-cache")
