...
export function ModelDetailPage() {
  ...
  const query = useQuery({
    queryKey: ["admin-model", modelId, context.toString()],
    queryFn: ({ signal }) => adminApi.model(modelId, context, signal),
  ...
  return (
    <DataBoundary data={query.data} error={query.error} pending={query.isPending}>
      {({ data, meta }) => {
        ...
        const stateNoticeClass = runtime.state === "unhealthy"
          ? "inline-notice--error"
          : runtime.state === "unknown" || runtime.state === "unsupported"
            ? "inline-notice--warning"
            : "";
        const modelStateSources = meta.sources.filter((source) => source.id === "kubernetes" || source.id === "prometheus");
        return (
          <div className="page-stack">
            <Link className="back-link" to={{ pathname: "/admin/models", search: context.toString() }}>← All models</Link>
            ...
            <div className={`inline-notice ${stateNoticeClass}`} role="status">
              <strong>Why this model is {runtime.state}:</strong> {runtime.reason} Catalog support is <code>{identity.support_state}</code> and the model is {identity.enabled ? "enabled" : "disabled"} in this cluster.
              {modelStateSources.length ? <span className="secondary-line">Observed sources: {modelStateSources.map((source) => `${source.id} ${source.state}`).join(" · ")}</span> : null}
            </div>
            <div className="metric-grid">
              ...
              <MetricCard label="Accepted-to-ready total (window)" value={metrics.cold_start_seconds} />
            </div>
