locals {
  ...
  foundation_variables = {
    ...
    pod_security_rollout_receipt = {
      ...
      custody_owner_username             = var.deployment.pod_security.receipt.custody_owner_username
      custody_owner_group                = var.deployment.pod_security.receipt.custody_owner_group
      platform_username                  = var.deployment.pod_security.receipt.platform_username
      platform_group                     = var.deployment.pod_security.receipt.platform_group
      external_handoff_path              = var.deployment.pod_security.receipt.external_handoff_path
      external_handoff_public_key_path   = var.deployment.pod_security.receipt.external_handoff_public_key_path
      external_handoff_public_key_sha256 = var.deployment.pod_security.receipt.external_handoff_public_key_sha256
      external_handoff_key_id            = var.deployment.pod_security.receipt.external_handoff_key_id
    }
    ...
    pod_security_storage_proof_generation = try(
      var.deployment.pod_security.successor_storage.proof_generation_ledger.active_generation,
      strrep("0", 64),
    )
    pod_security_storage_proof_attempt = try(
      var.deployment.pod_security.successor_storage.proof_generation_ledger.generations[var.deployment.pod_security.successor_storage.proof_generation_ledger.active_generation].attempt,
      0,
    )
    pod_security_storage_probe_image = try(
      var.deployment.pod_security.successor_storage.proof_generation_ledger.generations[var.deployment.pod_security.successor_storage.proof_generation_ledger.active_generation].probe_image,
      "prepare.invalid@sha256:${strrep("0", 64)}",
    )
    pod_security_storage_tools_config_map = "fs2-reference-data-tools-${substr(try(
      var.deployment.pod_security.successor_storage.proof_generation_ledger.generations[var.deployment.pod_security.successor_storage.proof_generation_ledger.active_generation].tools_data_sha256,
      strrep("0", 64),
    ), 0, 12)}"
  ...
  workloads_variables = {
    ...
    reference_data = {
      ...
      expected_tree_sha256 = var.deployment.storage.reference_data.expected_tree_sha256
    }
    scientific_artifacts = {
      enabled               = var.deployment.storage.scientific_artifacts.enabled
