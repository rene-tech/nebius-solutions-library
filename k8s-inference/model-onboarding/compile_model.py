...
def _scientific_adapter_runtime_recipe_sha256(
    ...
    for relative in sorted((*_SCIENTIFIC_ADAPTER_SHARED_RECIPE_PATHS, *model_paths)):
        ...
        if not path.is_file():
            raise OnboardingError(f"scientific adapter recipe input is missing: {relative}")
...
def _scientific_workload_projection(
    ...
    limitations = sorted(
        set(
            [
                "Candidate contract only: no Kubernetes/Kueue renderer, route, "
                "credential, or deployment is generated.",
