...
_SCIENTIFIC_ADAPTER_MODEL_RECIPE_PATHS = {
    "alphafold3": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/alphafold3.py", "catalog/runtime/schema/alphafold3-parameters.schema.json", "models/structure/runtime/alphafold3/adapter.py"),
    "boltzgen": (
    ...
    "esmfold2": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/esmfold2.py", "catalog/runtime/schema/esmfold2-parameters.schema.json", "models/structure/runtime/esmfold2/adapter.py"),
    "esmfold2-fast": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/esmfold2_fast.py", "catalog/runtime/schema/esmfold2-fast-parameters.schema.json", "models/structure/runtime/esmfold2-fast/adapter.py"),
    "openfold3": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/openfold3.py", "catalog/runtime/schema/openfold3-parameters.schema.json", "models/structure/runtime/openfold3/adapter.py"),
    "protenix-v2": ("components/control-plane/src/fs2_serve/scientific_batch/adapters/protenix_v2.py", "catalog/runtime/schema/protenix-v2-parameters.schema.json", "models/structure/runtime/protenix-v2/adapter.py"),
}
...
def _scientific_adapter_runtime_recipe_sha256(
    ...
    for relative in sorted((*_SCIENTIFIC_ADAPTER_SHARED_RECIPE_PATHS, *model_paths)):
        ...
        if not path.is_file():
            # Secondary adapter paths are reserved for their reviewed modules;
            # once present they are automatically included in the digest.
            continue
...
def _runtime_image(runtime: Mapping[str, Any]) -> tuple[str, str | None, str]:
    ...
    if not isinstance(repository, str) or _LOGICAL_REPOSITORY.fullmatch(repository) is None:
        raise OnboardingError(
    ...
    if state == "build-required":
        if digest is not None:
            raise OnboardingError("a build-required runtime image cannot declare a digest")
    elif state == "digest-pinned":
        if not isinstance(digest, str) or re.fullmatch(r"sha256:[0-9a-f]{64}", digest) is None:
            raise OnboardingError("a digest-pinned runtime image requires an immutable digest")
    else:
...
def _parameter_contract(
    ...
    if not isinstance(value, Mapping):
        raise OnboardingError("batch.parameter_schema must be an ID or an inline contract")
    ...
    try:
        ...
        raise OnboardingError(f"invalid batch parameter schema: {error.message}") from error
...
def _deployment_pool_contract(solution_root: Path) -> dict[str, Any]:
    ...
    path = (
        solution_root
        / "catalog/runtime/contracts/scientific-deployment-pools.json"
    )
...
def _custom_validate(declaration: Mapping[str, Any]) -> None:
    ...
    if _has_scientific_batch(declaration):
        ...
        if declaration["placement"].get("accelerator_resource_name") != "nvidia.com/gpu":
            raise OnboardingError(
...
def _scientific_workload_projection(
    ...
    poc_authorized = (
        batch.get("poc_authorization") == "user-authorized-academic-poc"
    )
    runtime_recipe = {
...
def _validate_collisions(declaration: Mapping[str, Any], solution_root: Path) -> None:
    ...
    if scientific_profiles_path.is_file():
        ...
        for profile in profiles.get("profiles", []):
            ...
            if default_endpoint is not None and existing_default_endpoint == default_endpoint:
