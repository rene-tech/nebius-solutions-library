...
class ModelOnboardingCompilerTests(unittest.TestCase):
    ...
    def test_cancer_candidate_declarations_compile_through_canonical_profile(self) -> None:
        ...
        for path in sorted(CANCER_DECLARATIONS.glob("*.json")):
            with self.subTest(declaration=path.name):
                ...
                self.assertFalse(profile["interface"]["mcp"]["invocable"])
                if profile["model_id"] in {"proteina-complexa", "boltzgen"}:
                    self.assertEqual(
                        "digest-pinned",
    ...
    def test_registered_scientific_recipe_rejects_missing_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaisesRegex(
                COMPILER.OnboardingError,
                "scientific adapter recipe input is missing",
            ):
                COMPILER._scientific_adapter_runtime_recipe_sha256(
                    Path(directory),
                    "esmfold2",
                )
    ...
    def test_hidden_localization_marker_is_allowed_but_traversal_is_rejected(self) -> None:
        declaration = COMPILER.load_declaration(CANCER_DECLARATIONS / "protenix-v2.json")
        required = declaration["model"]["artifacts"][0]["required_files"]
        self.assertIn(".fs2-manifest-sha256", required)
        required.append("../secret")
        with self.assertRaisesRegex(COMPILER.OnboardingError, "validation failed"):
            COMPILER._validate_declaration_value(declaration)
