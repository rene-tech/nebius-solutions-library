...
def render() -> bytes:
    ...
    for path in sorted(DECLARATION_ROOT.glob("*.json")):
        ...
        if repository in repositories:
            raise compile_model.OnboardingError(
                f"duplicate scientific runtime repository: {repository}"
            )
        if profile["default_variant"] and profile["model_id"] in default_models:
            raise compile_model.OnboardingError(
                f"duplicate default scientific variant: {profile['model_id']}"
            )
        pairs.add(pair)
        tools.add(tool)
    ...
    document = {
        ...
        "profiles": sorted(
            profiles, key=lambda item: (item["model_id"], item["variant_id"])
        ),
    }
...
def render_profile_set_schema() -> bytes:
    ...
    document = {
        ...
        "properties": {
            "schema": {
                "const": "fs2-serve.nebius.ai/scientific-workload-profiles/v1"
            },
...
def render_receipts() -> bytes:
    ...
    document = {
        ...
        "receipts": sorted(
            receipts, key=lambda item: (item["model_id"], item["variant_id"])
        ),
    }
    return (json.dumps(document, indent=2, sort_keys=True) + "\n").encode()
