...
def _preserved_legacy_items(path: Path, key: str) -> list[dict[str, object]]:
    """Preserve canonical primary profiles until they use variant declarations.

    This renderer owns only declarations in ``DECLARATION_ROOT``. Existing
    primary entries without ``variant_id`` remain canonical inputs from the
    primary-adapter lane and must not disappear when secondary declarations are
    synchronized.
    """
    ...
    if not isinstance(values, list):
        raise compile_model.OnboardingError(f"{path} has no {key} array")
    return [item for item in values if isinstance(item, dict) and "variant_id" not in item]


def render() -> bytes:
    profiles = _preserved_legacy_items(TARGET, "profiles")
    for profile in profiles:
        model_id = profile.get("model_id")
        if not isinstance(model_id, str):
            raise compile_model.OnboardingError("legacy scientific profile has no model_id")
        digest = compile_model._scientific_adapter_runtime_recipe_sha256(ROOT, model_id)
        identity = profile.get("execution_identity")
        if digest is None or not isinstance(identity, dict):
            raise compile_model.OnboardingError(f"legacy scientific profile {model_id} has no runtime recipe")
        identity["runtime_recipe_sha256"] = digest
    ...
    document = {
        ...
        "profiles": sorted(profiles, key=lambda item: (item["model_id"], item.get("variant_id", ""))),
    }
...
def render_receipts() -> bytes:
    receipts = _preserved_legacy_items(RECEIPT_TARGET, "receipts")
    for path in sorted(DECLARATION_ROOT.glob("*.json")):
        declaration = compile_model.load_declaration(path)
    ...
    document = {
        ...
        "receipts": sorted(receipts, key=lambda item: (item["model_id"], item.get("variant_id", ""))),
    }
    return (json.dumps(document, indent=2, sort_keys=True) + "\n").encode()
