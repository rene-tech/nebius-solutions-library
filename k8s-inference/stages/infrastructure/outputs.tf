...
output "reference_data_storage_contract" {
  ...
  value = var.reference_data.enabled ? {
    ...
    cpu_pool = {
      ...
      capacity             = "regular"
      schedulable_capacity = var.reference_data.cpu_pool.schedulable_capacity
      node_labels = {
...
output "reference_data_lifecycle" {
  description = "Truthful retention/adoption contract. Retained production blocks full-stack destroy even when downstream stages are released; disposable acceptance storage is deletable only while its versioned bucket is empty."
  value = var.reference_data.enabled ? {
    retention_mode = var.reference_data.lifecycle.retention_mode
...
output "postgresql_backup_storage_contract" {
  ...
  value = var.postgresql_backup.enabled ? {
    ...
    restore_reader = {
      service_account_id = nebius_iam_v1_service_account.postgresql_backup_restore[0].id
...
output "postgresql_backup_lifecycle" {
  ...
  value = var.postgresql_backup.enabled ? {
    ...
    resource_ids = {
      ...
      restore_service_account   = nebius_iam_v1_service_account.postgresql_backup_restore[0].id
      restore_group             = nebius_iam_v1_group.postgresql_backup_restore_readers[0].id
      restore_access_key        = nebius_iam_v2_access_key.postgresql_backup_restore[0].id
...
output "postgresql_backup_restore_object_storage_access" {
  description = "Sensitive non-secret identifier handoff for the dedicated read-only PostgreSQL PITR key."
  sensitive   = true
  value = var.postgresql_backup.enabled ? {
    key_id              = nebius_iam_v2_access_key.postgresql_backup_restore[0].id
    access_key_id       = nebius_iam_v2_access_key.postgresql_backup_restore[0].status.aws_access_key_id
    secret_reference_id = nebius_iam_v2_access_key.postgresql_backup_restore[0].status.secret_reference_id
    resource_version    = nebius_iam_v2_access_key.postgresql_backup_restore[0].resource_version
  } : null
}
