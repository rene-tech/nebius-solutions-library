...
if [ "$#" -ne 4 ]; then
  echo "usage: $0 ABSOLUTE_NEW_OUTPUT BOOTSTRAP_SHA256 ABSOLUTE_STATIC_FROZEN_PYTHON STATIC_FROZEN_PYTHON_SHA256" >&2
  exit 64
fi
...
case "$python_path" in
  /*) ;;
  *) echo "frozen Python path must be absolute" >&2; exit 64 ;;
esac
if [ -e "$output" ] || [ -L "$output" ]; then
  echo "refusing to replace existing launcher output" >&2
  exit 73
fi
for value in "$bootstrap_sha256" "$python_sha256"; do
...
exec /usr/bin/cc \
  ...
  -o "$output" "$source_file" -lcrypto -ldl -pthread
