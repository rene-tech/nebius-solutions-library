...
if [ "$#" -ne 4 ]; then
  echo "usage: $0 ABSOLUTE_NEW_OUTPUT INSTALLER_SOURCE_SHA256 STATIC_FROZEN_PYTHON_SHA256 FROZEN_RUNTIME_CLOSURE_REVIEW_SHA256" >&2
  exit 64
fi
output=$1
source_sha256=$2
...
for value in "$source_sha256" "$python_sha256" "$frozen_runtime_review_sha256"; do
...
exec /usr/bin/cc \
  ...
  -DFS2_EXPECTED_INSTALLER_FROZEN_RUNTIME_REVIEW_SHA256=\"$frozen_runtime_review_sha256\" \
  -Werror -Wall -Wextra -Wformat=2 \
  -Wl,-z,relro,-z,now,-z,noexecstack \
  -o "$output" "$script_dir/public-edge-capsule-installer-launcher.c"
