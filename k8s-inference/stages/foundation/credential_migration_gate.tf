...
resource "terraform_data" "credential_apply_gate_generation" {
  ...
  provisioner "local-exec" {
    command = "/usr/bin/python3 /opt/fs2/k8s-inference/scripts/secret_migration_guard.py apply-saved-plan-gate --terraform-configuration ${path.module} --terraform-root foundation --source-commit ${var.credential_migration_gate_source_commit}"
  }
