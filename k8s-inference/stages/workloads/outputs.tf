...
output "access_bundle" {
  ...
  value = {
    ...
    # One provider credential is materialized only into each tenant-isolated
    # broker. No provider credential exists in the shared gateway pod.
    scientific_artifacts = var.scientific_artifacts.enabled ? {
      ...
      object_roles        = var.scientific_artifacts.storage_contract.writer.roles
      credential_mode     = var.scientific_artifacts.storage_contract.writer.credential_mode
      broker_service_template = "fs2-system/${local.scientific_artifact_broker_name}-<sha256(tenant)[0:32]>"
      provider_keys_in_gateway = false
      tenant_broker_count      = length(local.scientific_artifact_tenant_access)
      provider_bindings_sha256 = sha256(local.scientific_artifact_provider_bindings_json)
...
output "scientific_artifacts_contract" {
  description = "Non-secret projection of the scientific result store: bucket identity, canonical object key, tenant-broker binding digest, exact chart values and batch gates."
  value       = terraform_data.scientific_artifacts_contract.output
}

output "scientific_artifacts_status" {
  description = "Non-secret result-store state: bucket identity, retention, per-tenant writer scope and isolated-broker binding."
  value = var.scientific_artifacts.enabled ? {
    ...
    object_key          = var.scientific_artifacts.storage_contract.layout.object_key
    object_roles        = var.scientific_artifacts.storage_contract.writer.roles
    tenant_principals   = var.scientific_artifacts.storage_contract.writer.tenant_principals
    credential_mode     = var.scientific_artifacts.storage_contract.writer.credential_mode
    lifecycle           = var.scientific_artifacts.storage_contract.lifecycle
    ...
    upload_handle_ttl_seconds = var.scientific_artifacts.upload_handle_ttl_seconds
    download_handle_ttl_seconds = var.scientific_artifacts.download_handle_ttl_seconds
