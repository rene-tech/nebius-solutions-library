...
# Customer-storage encryption and opaque-name keys advance together because a
# bucket row binds both identities. Each immutable Secret retains every reader
# needed for rollback while using only the selected generation for new writes.
resource "kubernetes_secret_v1" "storage_keyring_versioned" {
  ...
  type = "Opaque"
  data_wo = {
    "keyring.json"      = jsonencode(try(jsondecode(var.storage_keyrings_json[each.key]).cipher, null))
    "name-keyring.json" = jsonencode(try(jsondecode(var.storage_keyrings_json[each.key]).names, null))
  }
  data_wo_revision = tonumber(each.key)
  ...
  lifecycle {
    precondition {
      condition = (
        ...
        try(jsondecode(var.storage_keyrings_json[each.key]).cipher.active_key_id, "") == "storage-v${each.key}" &&
        try(jsondecode(var.storage_keyrings_json[each.key]).names.active_key_id, "") == "storage-name-v${each.key}" &&
        try(jsondecode(var.storage_keyrings_json[each.key]).cipher.keys["payload-v1"], "") == base64encode(random_password.key_material["payload"].result) &&
        try(jsondecode(var.storage_keyrings_json[each.key]).cipher.keys["storage-v1"], "") == base64encode(random_password.key_material["storage"].result) &&
        try(jsondecode(var.storage_keyrings_json[each.key]).names.keys["storage-name-v1"], "") == base64encode(random_password.key_material["storage_name"].result) &&
        try(toset(keys(jsondecode(var.storage_keyrings_json[each.key]).cipher.keys)), toset([])) == setunion(
          toset(["payload-v1"]),
          toset([for generation in range(1, tonumber(each.key) + 1) : "storage-v${generation}"])
        ) &&
        try(toset(keys(jsondecode(var.storage_keyrings_json[each.key]).names.keys)), toset([])) == toset([
          for generation in range(1, tonumber(each.key) + 1) : "storage-name-v${generation}"
        ])
      )
      error_message = "Each storage keyring must retain payload-v1 plus every storage cipher and name generation through its immutable active ID."
    }
    prevent_destroy = true
    ignore_changes  = all
  }
