...
variable "model_network_provider_gateway_members" {
  ...
  validation {
    ...
    error_message = "Live custody requires two to eight unique gateway members whose exact host routes cover the complete endpoint allowlist, whose measured listeners are exact non-loopback addresses/ports, and whose provider instance, firewall-rule, and IAM-permit identities are finite."
  }
}
