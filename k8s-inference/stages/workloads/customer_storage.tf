variable "customer_storage" {
  ...
  type = object({
    ...
    egress_boundary = optional(object({
      ...
      protected_observer_live_sha256      = optional(string, "")
      provider_authority = optional(object({
        schema                                 = optional(string, "")
        generation                             = optional(string, "")
        cluster_id                             = optional(string, "")
        provisioning_generation                = optional(string, "")
        provisioning_receipt_sha256             = optional(string, "")
        lane_id                                = optional(string, "")
        ...
        protected_observers = optional(map(object({
          class                 = string
          namespace             = string
          name                  = string
          uid                   = string
        ...
        protected_node_attestations                       = optional(map(any), {})
        protected_node_attestation_sha256                 = optional(string, "")
        ...
        daemonset_admission_fence_receipt_sha256          = optional(string, "")
        daemonset_snapshot_ledger_head_sha256             = optional(string, "")
        node_lifecycle_mode                               = optional(string, "")
        ...
        retained_legacy_workload_policies = optional(map(object({
          name          = string
          policy_sha256 = string
          policy_spec   = any
          binding_spec = object({
        ...
        retained_admission_custody_sha256       = optional(string, "")
  ...
  validation {
    condition = (
      ...
      (!var.customer_storage.enabled || (
        ...
        can(regex("^[a-f0-9]{64}$", var.customer_storage.egress_boundary.provider_authority.provisioning_receipt_sha256)) &&
        ...
        can(regex("^[a-f0-9]{64}$", var.customer_storage.egress_boundary.provider_authority.daemonset_admission_fence_receipt_sha256)) &&
        can(regex("^[a-f0-9]{64}$", var.customer_storage.egress_boundary.provider_authority.daemonset_snapshot_ledger_head_sha256)) &&
        var.customer_storage.egress_boundary.provider_authority.node_lifecycle_mode == "GENERATIONAL_SINGLETON_RETAIN_PREDECESSOR" &&
        ...
        can(regex("^[a-f0-9]{64}$", var.customer_storage.egress_boundary.provider_authority.retained_admission_custody_sha256)) &&
        sha256(jsonencode({
          retained_legacy_boundary_policies = var.customer_storage.egress_boundary.provider_authority.retained_legacy_boundary_policies
          retained_legacy_workload_policies = var.customer_storage.egress_boundary.provider_authority.retained_legacy_workload_policies
        ...
        alltrue([
          ...
          (
            (
              try(identity.kind, "") == "ServiceAccount" && try(identity.namespace, "") == "kube-system" &&
              try(identity.username, "") == "system:serviceaccount:kube-system:${try(identity.name, "")}" &&
              can(regex("^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$", try(identity.uid, ""))) &&
              try(identity.groups, []) == ["system:authenticated", "system:serviceaccounts", "system:serviceaccounts:kube-system"]
            ) || (
              try(identity.kind, "") == "User" && try(identity.namespace, "") == "" &&
              startswith(try(identity.name, ""), "system:") && try(identity.username, "") == try(identity.name, "") &&
              try(identity.uid, "") != "" && try(identity.groups, []) == ["system:authenticated"]
            )
          ) &&
        ...
        var.customer_storage.egress_boundary.provider_authority.node_selector_value == var.customer_storage.egress_boundary.provider_authority.lane_id &&
        ...
        toset(keys(var.customer_storage.egress_boundary.provider_authority.protected_node_attestations)) == toset(var.customer_storage.egress_boundary.provider_authority.protected_node_names) &&
        alltrue([
        ...
        sha256(jsonencode(var.customer_storage.egress_boundary.provider_authority.protected_node_attestations)) == var.customer_storage.egress_boundary.provider_authority.protected_node_attestation_sha256 &&
        length(var.customer_storage.egress_boundary.provider_authority.protected_observers) > 0 &&
        alltrue([
...
data "external" "customer_storage_integration_dependencies" {
  ...
  query = {
    ...
    expected_dependencies_json = jsonencode({
      ...
      retained_admission_custody_sha256                 = var.customer_storage.egress_boundary.provider_authority.retained_admission_custody_sha256
...
data "kubernetes_resource" "customer_storage_retained_legacy_boundary_policy" {
  for_each    = var.customer_storage.enabled ? var.customer_storage.egress_boundary.provider_authority.retained_legacy_boundary_policies : {}
  api_version = "admissionregistration.k8s.io/v1"
  kind        = "ValidatingAdmissionPolicy"
  metadata { name = each.value.name }
}

data "kubernetes_resource" "customer_storage_retained_legacy_boundary_binding" {
  for_each    = var.customer_storage.enabled ? var.customer_storage.egress_boundary.provider_authority.retained_legacy_boundary_policies : {}
  api_version = "admissionregistration.k8s.io/v1"
  kind        = "ValidatingAdmissionPolicyBinding"
  metadata { name = each.value.name }
...
data "kubernetes_resource" "customer_storage_retained_legacy_workload_policy" {
  for_each    = var.customer_storage.enabled ? var.customer_storage.egress_boundary.provider_authority.retained_legacy_workload_policies : {}
  api_version = "admissionregistration.k8s.io/v1"
  kind        = "ValidatingAdmissionPolicy"
  metadata { name = each.value.name }
}

data "kubernetes_resource" "customer_storage_retained_legacy_workload_binding" {
  for_each    = var.customer_storage.enabled ? var.customer_storage.egress_boundary.provider_authority.retained_legacy_workload_policies : {}
  api_version = "admissionregistration.k8s.io/v1"
  kind        = "ValidatingAdmissionPolicyBinding"
  metadata { name = each.value.name }
...
resource "terraform_data" "customer_storage_external_egress_boundary" {
  ...
  lifecycle {
    ...
    precondition {
      condition = alltrue([
        for generation, expected in var.customer_storage.egress_boundary.provider_authority.retained_legacy_boundary_policies :
        try(data.kubernetes_resource.customer_storage_retained_legacy_boundary_policy[generation].object.spec, null) == expected.policy_spec &&
        sha256(jsonencode(try(data.kubernetes_resource.customer_storage_retained_legacy_boundary_policy[generation].object.spec, null))) == expected.policy_sha256 &&
        try(data.kubernetes_resource.customer_storage_retained_legacy_boundary_binding[generation].object.spec, null) == expected.binding_spec
      ])
      error_message = "A retained legacy customer-storage boundary VAP or Deny binding drifted from signed custody."
    }
    precondition {
      condition = alltrue([
        for generation, expected in var.customer_storage.egress_boundary.provider_authority.retained_legacy_workload_policies :
        try(data.kubernetes_resource.customer_storage_retained_legacy_workload_policy[generation].object.spec, null) == expected.policy_spec &&
        sha256(jsonencode(try(data.kubernetes_resource.customer_storage_retained_legacy_workload_policy[generation].object.spec, null))) == expected.policy_sha256 &&
        try(data.kubernetes_resource.customer_storage_retained_legacy_workload_binding[generation].object.spec, null) == expected.binding_spec
      ])
      error_message = "A retained legacy customer-storage workload VAP or Deny binding drifted from signed custody."
    }
    precondition {
      condition = alltrue([
