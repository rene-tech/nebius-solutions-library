...
if kubectl --kubeconfig "${kubeconfig}" apply --server-side --dry-run=server -f - >/dev/null 2>"${work_dir}/immutable-localqueue.log" <<'YAML'
apiVersion: kueue.x-k8s.io/v1beta2
kind: LocalQueue
metadata:
  name: inference-models
...
grep -Eqi 'immutable|may not be changed|Invalid value' "${work_dir}/immutable-localqueue.log"
