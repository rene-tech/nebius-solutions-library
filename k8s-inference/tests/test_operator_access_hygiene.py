...
class OperatorAccessHygieneTests(unittest.TestCase):
    ...
    def test_key_classes_rotate_independently_with_retained_v1_and_write_only_delivery(
        ...
        variables = (ROOT / "stages/workloads/variables.tf").read_text(encoding="utf-8")
        wrapper = (ROOT / "inference-stack").read_text(encoding="utf-8")
        for key_class in ("payload", "storage", "ledger", "pepper", "attestor"):
        ...
        storage = dict(hcl_blocks(secrets, "resource"))["storage_keyring"]
        self.assertIn("data_wo", storage)
        self.assertIn("data_wo_revision", storage)
        self.assertIn("prevent_destroy = true", storage)
        self.assertIn("terraform_data.credential_migration_gate", storage)
        self.assertIn('"storage_keyrings_json": "FS2_STORAGE_KEYRINGS_JSON"', wrapper)
    ...
    def test_secret_consumers_have_nonsecret_generation_rollout_triggers(self) -> None:
        ...
        for name in (
            ...
            "storage-disclosure-deployment.yaml",
        ):
        ...
        self.assertIn(
            '"storage" .Values.secretRollout.storageGeneration',
            templates["storage-reconciler-deployment.yaml"],
        )
        self.assertIn(
            '"storage" .Values.secretRollout.storageGeneration',
            templates["storage-disclosure-deployment.yaml"],
        )
    ...
    def test_storage_predecessor_disable_requires_zero_cipher_and_name_references(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = ROTATION.private_directory(Path(temporary) / "rotation")
            base_identity = {
                "credential_class": "customer-storage-keyring",
                "owner_id": "owner-test",
                "project_id": "project-test",
                "purpose": "customer-storage credential encryption and opaque bucket naming",
            }
            journal = {
                "schema": "fs2-serve.nebius.ai/credential-rotation/v2",
                "operation_id": "operation-test",
                **base_identity,
                "readers": ["storage-reconciler", "storage-disclosure"],
                "registry_sha256": "a" * 64,
                "predecessor": {
                    "id": "storage-v1",
                    **base_identity,
            ...
            incomplete = {
                ...
                "old_cipher_references": 0,
                "old_name_references": 1,
                "legacy_name_references": 2,
                "credential_rows": 2,
                "bucket_rows": 2,
                "current_write_id": "storage-v2",
            }
            with (
                mock.patch.object(ROTATION, "provider_call", return_value=incomplete),
                self.assertRaisesRegex(ROTATION.RotationError, "cipher/name"),
            ):
                ROTATION.transition(args)

            complete = {**incomplete, "old_name_references": 0}
            disabled = {**journal["predecessor"], "status": "disabled"}
            with mock.patch.object(
                ROTATION, "provider_call", side_effect=[complete, disabled]
            ):
                result = ROTATION.transition(args)
            self.assertEqual(result["status"], "predecessor-disabled")
