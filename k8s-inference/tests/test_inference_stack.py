...
class InferenceStackTests(unittest.TestCase):
    ...
    def test_internal_proxy_command_rejects_operator_owned_raw_listeners(
        self,
    ) -> None:
        with self.assertRaisesRegex(
            STACK.DeploymentError, "operator-owned port-forward listeners are forbidden"
        ):
            STACK.internal_proxy_command(
                arguments(), Path("/private/test-run"), contract()
            )

    def test_internal_proxy_retirement_is_unconditional(self) -> None:
        with self.assertRaisesRegex(STACK.DeploymentError, "broker-owned"):
            STACK.internal_proxy_command(
                arguments(), Path("/private/test-run"), contract()
            )
