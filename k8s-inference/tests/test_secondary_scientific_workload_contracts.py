...
class ScientificWorkloadContractTests(unittest.TestCase):
    ...
    def test_source_receipts_match_qualified_exact_pins(self) -> None:
        ...
        self.assertEqual(
            len(actual),
            len([item for item in receipts["receipts"] if "variant_id" in item]),
        )
        self.assertTrue(
            all(item["status"] == "candidate" for item in receipts["receipts"] if "variant_id" in item)
        )
        self.assertTrue(
            all(
    ...
    def test_compiled_candidate_profiles_are_canonical_and_non_invocable(self) -> None:
        ...
        actual = set()
        for profile in (item for item in profiles["profiles"] if "variant_id" in item):
