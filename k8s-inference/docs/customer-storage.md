...
customer_storage = {
  ...
  resource_credentials_generation  = 1
  iam_credentials_generation       = 1
}
```
...
uv run --project components/control-plane python \
  ...
  --create --private-key "$OWNER_ONLY_EGRESS_SIGNING_KEY" \
...
only a `credentials.json` key to the reconciler. Increment the matching
non-secret generation whenever either Secret's data changes; the storage
reconciler pod template hashes both generations and rolls atomically. Do not
manage their private contents in this module.
...
6. Rotate both provisioner JWT keys before expiry, update the two external
   Secrets, increment their independent generations, wait for the reconciler
   rollout, and verify no public pod mounts them.
7. Advance the independently versioned storage keyring only after its bundle
   retains every storage and opaque-name predecessor plus every payload key
   still reported by `payload_key_usage()`. Prove old customer-storage rows
   decrypt under their original tenant/principal AAD, re-encrypt them under the
   current storage key, and require a zero usage count before removing any
   historical payload key in a later reviewed generation.
