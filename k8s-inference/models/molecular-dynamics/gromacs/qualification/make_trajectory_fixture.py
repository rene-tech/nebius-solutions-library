...
def main():
    ...
    if receipt['state'] != 'verified':
        raise ValueError('Use the verified completed fixture, not an incomplete transfer.')
    entries = json.loads((args.receipt / 'output-manifest.json').read_text())['entries']
    by_hash = {item['artifact']['sha256']: args.receipt / f'output-{index:02d}.artifact'
               for index, item in enumerate(entries)}
    result = next(json.loads(by_hash[item['artifact']['sha256']].read_text()) for item in entries
                  if item['semantic_type'] == 'gromacs-workflow-result/v1')
    selected = {item['path']: item for item in result['files'] if item['path'] in {'md.trr', 'md.tpr'}}
    ...
    request = {'schema': 'fs2-serve.nebius.ai/gromacs-workflow-request/v1',
               'jobs': [{'id': 'trajectory-analysis', 'steps': [
                   {'id': 'check-input', 'command': 'check', 'args': ['-f', 'md.trr']},
                   {'id': 'backbone-rmsd', 'command': 'rms',
                    'args': ['-s', 'md.tpr', '-f', 'md.trr', '-o', 'rmsd.xvg', '-tu', 'ns'],
                    'stdin': 'Backbone\nBackbone\n', 'expected_outputs': ['rmsd.xvg']},
               ]}]}
    (args.output / 'request.json').write_text(json.dumps(request, indent=2) + '\n')
    summary = {'source_operation_id': result['operation_id'], 'native_files': selected,
    ...
    assert summary['input_bytes'] > 16 * 1024**2, 'This fixture must exercise the live presigned input path.'
    (args.output / 'fixture.json').write_text(json.dumps(summary, indent=2) + '\n')
    print(json.dumps({'output': str(args.output), 'input_bytes': summary['input_bytes']}))
