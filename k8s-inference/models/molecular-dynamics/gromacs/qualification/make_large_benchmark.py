...
def main():
    ...
    with args.archive.open("rb") as source:
        assert (
            hashlib.file_digest(source, "md5").hexdigest()
            == "3b142fe31972077474967636ab4c69e0"
        )
    ...
    with tarfile.open(args.archive, "r:gz") as archive:
        members = [
            ...
            if member.name.endswith("/stmv/topol.tpr")
        ]
        if (
            len(members) != 1
            or not members[0].isfile()
            or members[0].size > 256 * 1024**2
        ):
            raise ValueError(
                "Expected one bounded regular STMV input; inspect the pinned archive."
            )
        with (
            archive.extractfile(members[0]) as source,
            (data / "source.tpr").open("xb") as target,
        ):
    ...
    steps.extend(
        {
            "id": f"benchmark-{index + 1}",
            "command": "mdrun",
            "args": ["-s", "md.tpr", "-deffnm", f"md-{index + 1}"],
        }
        for index in range(args.repetitions)
    )
    request = {
        "schema": "fs2-serve.nebius.ai/gromacs-workflow-request/v1",
        "jobs": [{"id": "stmv", "steps": steps}],
    ...
    (args.output / "fixture.json").write_text(
        json.dumps(
            {
                "source": "https://doi.org/10.5281/zenodo.3893789",
                "source_tpr_sha256": sha,
                "steps": args.steps,
                "repetitions": args.repetitions,
                "scientific_protocol_changed": False,
                "run_length_changed_for_benchmark": True,
            },
            indent=2,
        )
        + "\n"
