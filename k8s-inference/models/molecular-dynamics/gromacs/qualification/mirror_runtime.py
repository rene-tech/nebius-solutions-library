...
def main():
    secret = json.loads(subprocess.check_output(KUBE + ["get", "secret", "wan2-ngc-pull", "-o", "json"]))
    config = json.loads(base64.b64decode(secret["data"][".dockerconfigjson"]))
    credential = json.loads(subprocess.check_output(
        ["docker-credential-nebius", "get"], input=(HOST + "\n").encode(),
    ))
    config["auths"][HOST] = {
        "auth": base64.b64encode((credential["Username"] + ":" + credential["Secret"]).encode()).decode()
    }
    objects = [{
        "apiVersion": "v1", "kind": "Secret",
        "metadata": {"name": NAME, "namespace": "fs2-models"},
        "type": "Opaque", "stringData": {"config.json": json.dumps(config)},
    }, {
        "apiVersion": "batch/v1", "kind": "Job",
        "metadata": {"name": NAME, "namespace": "fs2-models"},
        "spec": {
            "backoffLimit": 0, "activeDeadlineSeconds": 900,
            "template": {
                ...
                "spec": {
                    ...
                    "containers": [{
                        ...
                        "command": ["crane"], "args": ["copy", SOURCE, TARGET],
    ...
    subprocess.run(KUBE + ["apply", "-f", "-"], input=json.dumps({
