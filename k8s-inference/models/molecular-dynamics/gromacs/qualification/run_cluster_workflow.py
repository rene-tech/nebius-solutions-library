...
def main():
    ...
    parser.add_argument(
        "--pool",
        choices=("h100-ondemand-1x", "h100-reserved-8x", "l40s-1x"),
        required=True,
    )
    ...
    parser.add_argument(
        "--existing-pod",
        help="Reuse only an existing task-owned qualification Pod with the same image and pool.",
    )
    args = parser.parse_args()
    if "@sha256:" not in args.image:
        raise ValueError("pin the candidate image digest")
    jobs = json.loads((args.input / "request.json").read_text())["jobs"]
    if args.job_id is None and len(jobs) == 1:
        args.job_id = jobs[0]["id"]
    if args.job_id not in {job["id"] for job in jobs}:
        raise ValueError("Choose a job ID from the fixture before allocating a GPU Pod")
    args.output.mkdir(parents=True, exist_ok=False)
    name = "fs2-gromacs-r20260923-" + args.pool.split("-")[0] + "-" + args.suffix
    if args.existing_pod:
        name = args.existing_pod
        existing = json.loads(
            subprocess.check_output(KUBE + ["get", "pod", name, "-o", "json"])
        )
        if (
            existing["metadata"].get("labels", {}).get("scientific-ai.nebius.com/task")
            != "gromacs-r20260923"
            or existing["spec"]["containers"][0]["image"] != args.image
            or existing["spec"]["nodeSelector"]["accelerator.fs2.nebius/pool-id"]
            != args.pool
        ):
            raise ValueError(
                "reuse is restricted to the matching task-owned qualification Pod"
            )
    ...
    if not args.existing_pod:
        subprocess.run(
            KUBE + ["apply", "-f", "-"], input=json.dumps(pod).encode(), check=True
        )
    subprocess.run(
    ...
    subprocess.run(
        KUBE + ["cp", "--no-preserve", str(args.input), name + ":" + remote], check=True
    )
    (args.output / "environment.txt").write_bytes(
    ...
    subprocess.run(
        KUBE + ["cp", name + ":" + remote, str(args.output / "workspace")], check=True
    )
    ...
    record = {
        "image": args.image,
        "pod": name,
        "node": pod_state["spec"]["nodeName"],
        "pool": args.pool,
        "pod_ready_seconds": ready_seconds,
        "runtime_wall_seconds": execution_seconds,
        "worker_exit_code": result.returncode,
        "customer_path_tested": False,
        "checkpoint_transport": "local-only",
        "cpus": 8,
        "gpu_count": 1,
        "pod_reused": bool(args.existing_pod),
    }
