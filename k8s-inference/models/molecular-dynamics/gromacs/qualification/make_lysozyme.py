...
def main():
    ...
    protein = (
        "\n".join(
            line for line in text.splitlines() if line.startswith(("ATOM  ", "TER   "))
        )
        + "\nEND\n"
    )
    ...
    (data / "ions.mdp").write_text(
        EM.replace("coulombtype = PME", "coulombtype = Cut-off")
    )
    (data / "nvt.mdp").write_text(
        MD + "nsteps = 10000\ngen-vel = yes\ngen-temp = 300\ngen-seed = 20260923\n"
    )
    (data / "md.mdp").write_text(
        MD + f"nsteps = {args.steps}\ncontinuation = yes\ngen-vel = no\n"
    )
    steps = []

    def step(name, command, argv, stdin=""):
        steps.append(
            {"id": name, "command": command, "args": argv.split(), "stdin": stdin}
        )

    step(
        "topology",
        "pdb2gmx",
        "-f protein.pdb -o protein.gro -p topol.top -ff amber99sb-ildn -water tip3p -ignh",
    )
    step("box", "editconf", "-f protein.gro -o box.gro -c -d 1.0 -bt dodecahedron")
    step(
        "solvate", "solvate", "-cp box.gro -cs spc216.gro -o solvated.gro -p topol.top"
    )
    step("ions-tpr", "grompp", "-f ions.mdp -c solvated.gro -p topol.top -o ions.tpr")
    step(
        "neutralize",
        "genion",
        "-s ions.tpr -o solvated-ions.gro -p topol.top -pname NA -nname CL -neutral -seed 20260923",
        "SOL\n",
    )
    ...
    step(
        "md-tpr",
        "grompp",
        "-f md.mdp -c nvt.gro -t fs2-equilibrate.cpt -p topol.top -o md.tpr",
    )
    ...
    step(
        "backbone-rmsd",
        "rms",
        "-s md.tpr -f md.xtc -o backbone-rmsd.xvg",
        "Backbone\nBackbone\n",
    )
    step(
        "energies", "energy", "-f md.edr -o energies.xvg", "Potential\nTemperature\n0\n"
    )
    request = {
        "schema": "fs2-serve.nebius.ai/gromacs-workflow-request/v1",
        "jobs": [{"id": "lysozyme", "steps": steps}],
        "threads": 8,
        "segment_minutes": 0.1,
    ...
    (args.output / "source.json").write_text(
        json.dumps(
            {
                "source": SOURCE,
                "source_sha256": hashlib.sha256(pdb).hexdigest(),
