...
def main():
    ...
    def checkpoint(*options):
        ...
        result = subprocess.run([str(args.checkpoint_tool), *options, '--pid', str(child.pid)],
                                text=True, capture_output=True, timeout=30)
        events.append({'options': options, 'seconds': time.monotonic() - started,
                       'exit_code': result.returncode, 'stdout': result.stdout, 'stderr': result.stderr})
        return result

    started = time.monotonic()
    with (args.output / 'mdrun.stdout.log').open('wb') as output:
        child = subprocess.Popen(['/usr/local/gromacs/avx2_256/bin/gmx', 'mdrun', '-s', 'run.tpr',
            '-deffnm', 'md', '-ntmpi', '1', '-ntomp', '8', '-pin', 'off'], cwd=args.output,
            env={**os.environ, 'OMP_NUM_THREADS': '8'}, stdout=output, stderr=subprocess.STDOUT,
        ...
        try:
            ...
            if child.poll() is None:
                ...
                try:
                    ...
                    child.wait()
            record = {'pid': child.pid, 'worker_exit_code': child.returncode,
                'wall_seconds': time.monotonic() - started, 'events': events,
                'tool_sha256': hashlib.sha256(args.checkpoint_tool.read_bytes()).hexdigest(),
                'tpr_sha256': hashlib.sha256(args.tpr.read_bytes()).hexdigest(),
                'durable_process_snapshot_tested': False, 'customer_path_tested': False}
