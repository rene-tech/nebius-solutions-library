...
def main():
    ...
    command = ['docker', 'run', '--rm', '--user', f'{os.getuid()}:{os.getgid()}',
               ...
               '--mount', f'type=bind,src={args.fixture.resolve()},dst=/qualification/input,readonly',
               '--mount', f'type=bind,src={args.output.resolve()},dst=/qualification/receipt', args.image,
               '/opt/bionemo/invoke-scientific-batch.py', '--model', 'gromacs',
               '--tool', 'submit_gromacs_workflow', '--operation', 'run-workflow',
               '--source', '/qualification/input/input.tar.gz', '--parameters', '/qualification/input/request.json',
               '--entry-name', 'gromacs-inputs', '--semantic-type', 'gromacs-input-bundle/v1',
               '--media-type', 'application/x-tar', '--compression', 'gzip',
               '--output', '/qualification/receipt', '--idempotency-key', args.idempotency_key,
               '--display-name', 'GROMACS exact-workbench-image qualification',
