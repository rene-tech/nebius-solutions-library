...
def unpack(response):
    ...
    if data.get('isError'):
        raise RuntimeError('The customer MCP tool returned an error; retain the operation for diagnosis.')
...
async def run(args):
    if args.output.exists():
        raise ValueError('This cancellation test already has a receipt; do not repeat it blindly.')
    key = json.loads(args.key_file.read_text())
    deadline = time.monotonic() + 900
    status_file = args.receipt / 'status.json'
    async with httpx2.AsyncClient(headers={'authorization': 'Bearer ' + key['secret']},
                                  timeout=90, trust_env=False) as http:
        async with Client(streamable_http_client('https://89.169.99.188/mcp', http_client=http)) as client:
            while time.monotonic() < deadline:
                ...
                if operation['status'] in {'failed', 'cancelled', 'succeeded'}:
                    raise RuntimeError('Operation ended before the running-cancellation test.')
                ...
                record = {'operation_id': operation['id'], 'requested_at': requested_at.isoformat(),
                          'before': status, 'tool': 'cancel_scientific_run'}
                with os.fdopen(os.open(args.output, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), 'w') as f:
                    json.dump(record, f, indent=2)
                record['response'] = unpack(await client.call_tool('cancel_scientific_run',
                    {'operation_id': operation['id']}))
                while time.monotonic() < deadline:
                    final = unpack(await client.call_tool('get_scientific_status',
                        {'operation_id': operation['id']}))
                    attempts = [a for s in final['batch']['stages'] for a in s['attempts']]
                    if final['operation']['status'] == 'cancelled' and all(a['resource_released'] for a in attempts):
                        kube = ['kubectl', '--kubeconfig', args.kubeconfig, '--context', args.context,
                                '-n', 'fs2-models']
                        remaining = []
                        for attempt in attempts:
                            pods = json.loads(subprocess.check_output(kube + ['get', 'pods', '-l',
                                'batch.kubernetes.io/job-name=' + attempt['workload_name'], '-o', 'json']))['items']
                            remaining.extend(p['metadata']['name'] for p in pods)
                        if not remaining:
                            record.update(final=final, remaining_pods=remaining,
                                seconds_to_gpu_release=(datetime.now(timezone.utc) - requested_at).total_seconds())
                            # This is a generated test receipt, not a source-file edit.
                            args.output.write_text(json.dumps(record, indent=2) + '\n')
                            print(json.dumps({k: record[k] for k in
                                ('operation_id', 'seconds_to_gpu_release', 'remaining_pods')}))
                            return
                    await asyncio.sleep(3)
    raise TimeoutError('Cancellation or resource release did not complete; inspect the saved operation.')
