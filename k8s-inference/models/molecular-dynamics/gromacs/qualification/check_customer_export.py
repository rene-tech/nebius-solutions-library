...
def main():
    ...
    with httpx.Client(
        base_url="https://89.169.99.188", headers={"Authorization": "Bearer " + key}
    ) as http:
        response = http.post("/v1/storage/credentials")
        if response.status_code != 200:
            raise RuntimeError(
                f"Credential disclosure failed: HTTP {response.status_code}"
            )
        value = response.json()
    s3 = boto3.client(
        "s3",
    ...
    value.clear()
    prefix = f"runs/{args.model}/{args.operation_id}/"
    rows = [
        row
    ...
    def verify(row):
        ...
        with obj["Body"] as body:
            while chunk := body.read(4 * 1024**2):
                ...
                if manifest is not None:
                    if size > 16 * 1024**2:
                        raise ValueError(
                            "Unexpectedly large customer checkpoint manifest."
                        )
        ...
        if manifest is not None:
            ...
            metadata = [
                v for k, v in head.get("Metadata", {}).items() if k.lower() == "sha256"
            ]
            assert (
                expected == result["sha256"]
                and metadata
                and all(v == expected for v in metadata)
            ), row["Key"]
        return result
    ...
    for item in verified:
        ...
        if manifest is None:
            continue
        assert (
            manifest["schema"] == "fs2-serve.nebius.ai/gromacs-customer-checkpoint/v1"
        )
        ...
        for entry in manifest["files"]:
            ...
            assert (
                ...
                and stored["size_bytes"] == entry["size_bytes"]
            )
            file_references += 1
        manifests += 1
    ...
    summary = {
        ...
        "file_references_verified": file_references,
    }
    with os.fdopen(
