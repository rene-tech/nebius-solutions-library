...
def main():
    ...
    parser.add_argument("--binary", default=GMX)
    ...
    (args.output / "version.txt").write_bytes(
        subprocess.check_output([args.binary, "--version"], stderr=subprocess.STDOUT)
    )
    (args.output / "gpu.csv").write_bytes(
    ...
    for layout, options in (
        ...
        for repetition in range(args.repetitions):
            ...
            argv = [
                ...
                "-deffnm",
                "md",
                *([] if args.external_mpi else ["-ntmpi", "1"]),
                "-ntomp",
                "8",
                *options,
            ]
            with (
                (directory / "gpu-samples.csv").open("wb") as samples,
                (directory / "command.log").open("wb") as log,
            ):
