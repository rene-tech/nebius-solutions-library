...
def main():
    ...
    for index, item in enumerate(manifest['entries']):
        ...
        assert digest == item['artifact']['sha256'] and source.stat().st_size == item['artifact']['size_bytes']
        by_digest[digest] = source
        if item['semantic_type'] == 'gromacs-workflow-result/v1':
            results.append(json.loads(source.read_text()))
    requested = {job['id']: job for job in json.loads(args.parameters.read_text())['jobs']}
    assert {result['job_id'] for result in results} == set(requested) and len(results) == len(requested)
    verified = []
    for result in results:
        assert result['operation_id'] == status['operation']['id'] and result['status'] == 'succeeded'
        assert result['completed_steps'] == [step['id'] for step in requested[result['job_id']]['steps']]
        ...
        for item in result['files']:
            ...
            assert source.stat().st_size == item['size_bytes'] and item['path'] not in paths
    ...
    evidence = {'operation_id': operation['id'], 'accepted_to_completed_seconds': elapsed,
                'jobs': verified, 'convergence_claimed': False, 'gpu_snapshot_used': False}
