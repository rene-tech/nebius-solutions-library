...
def main():
    ...
    with os.fdopen(os.open(render_values, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), 'w') as output:
        json.dump(merged, output)
    rendered = subprocess.check_output(helm + ['template', 'fs2-serve-control-plane', str(chart), '-f', str(render_values)])
    with os.fdopen(os.open(args.activation / 'rendered.yaml', os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), 'wb') as output:
        output.write(rendered)
    cm = args.activation / 'scheduling.configmap.json'
    subprocess.run(kube + ['apply', '--dry-run=server', '-f', str(cm)], check=True)
    print(json.dumps({'rendered': True, 'existing_apps_preserved': len(source['models']) - 1,
                      'previous_digest': baseline['image']['digest'], 'next_digest': args.image_digest,
