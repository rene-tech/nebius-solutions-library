...
def main():
    ...
    cm = json.loads(
        subprocess.check_output(
            ...
            + [
                "-n",
                config["schedulingContractNamespace"],
                "get",
                "configmap",
                config["schedulingContractConfigMapName"],
                "-o",
                "json",
            ]
        )
    ...
    for name, content in [
        ...
        with os.fdopen(
            os.open(args.output / name, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600),
            "wb",
        ) as handle:
    ...
    print(
        json.dumps(
            {
                "image": image,
                "release_capture": str(args.output),
