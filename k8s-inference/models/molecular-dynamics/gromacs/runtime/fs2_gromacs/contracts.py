...
def request_schema(*, mpi: bool = False) -> dict[str, Any]:
    step = {
        "type": "object",
        ...
        "properties": {
            ...
            "directory": {
                **RELATIVE,
                "default": ".",
                "description": "Working directory relative to the extracted input bundle.",
            },
            "stdin": {
                "type": "string",
                "maxLength": 16384,
            ...
            "restart_checkpoint": {
                **RELATIVE,
                "description": "Optional native .cpt path relative to directory. Missing files are errors, never a silent fresh simulation.",
            },
            "plumed_input": {
                **RELATIVE,
                "description": "PLUMED input file relative to directory for an mdrun step. Uses the packaged kernel; GROMACS passes native checkpoint/restart events to PLUMED. Include all bias/restart files in the input bundle when continuing an external run.",
            },
            "expected_outputs": {
                "type": "array",
                "maxItems": 256,
    ...
    if mpi:
        # The separately compiled MPI engine has no qualified parallel PLUMED
        # kernel. Do not advertise an input that can only fail after admission.
        ...
        schema["$id"] = (
            "https://fs2-serve.nebius.ai/schema/gromacs-mpi-workflow-request/v1"
        )
        schema["properties"]["schema"] = {
            "const": "fs2-serve.nebius.ai/gromacs-mpi-workflow-request/v1"
        }
        schema["properties"]["jobs"]["maxItems"] = 1
        schema["properties"]["jobs"]["items"]["properties"]["id"] = {"const": "gang"}
        schema["properties"]["jobs"]["description"] = (
            "One distributed workflow with ID gang; every mdrun uses the complete admitted MPI gang. Preparation and analysis run once on rank zero."
        )
...
def normalize(value: object, *, mpi: bool = False) -> dict[str, Any]:
    Draft202012Validator(request_schema(mpi=mpi)).validate(value)
    ...
    for job in result["jobs"]:
        ...
        for step in job["steps"]:
            ...
            if "plumed_input" in step:
                if step["command"] != "mdrun":
                    raise ValueError("plumed_input is only valid for mdrun")
                relative_path(step["plumed_input"])
            ...
            for token in step["args"]:
                if isinstance(token, dict):
                    ...
                    continue
                if (
                    any(ord(c) < 32 for c in token)
                    or token.startswith("/")
                    or "\\" in token
                    or re.search(r"(^|[/=\s])\.\.([/\s]|$)", token)
                ):
                    raise ValueError(
                        "arguments must use workspace-relative paths without control characters"
                    )
