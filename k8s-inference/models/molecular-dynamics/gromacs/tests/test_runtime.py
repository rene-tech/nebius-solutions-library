...
from fs2_gromacs.worker import (
    Workflow,
    expand_args,
    parse_mdp,
    publish_final_coordinates,
)
...
@pytest.mark.parametrize(
    "argument", ["-ntmpi", "-maxh", "-cpi", "-multidir", "-replex", "-plumed"]
)
...
@pytest.mark.parametrize(
    "name", ["../outside", "/absolute", "dir/../file", "data\\file", "x\x00y"]
)
...
def test_plumed_has_a_typed_workspace_input_not_a_customer_library():
    body = request()
    step = body["jobs"][0]["steps"][0]
    step["plumed_input"] = "bias/plumed.dat"
    assert normalize(body)["jobs"][0]["steps"][0]["plumed_input"] == "bias/plumed.dat"
    step["plumed_input"] = "/tmp/other.dat"
    with pytest.raises(ValueError, match="relative"):
        normalize(body)
    step["plumed_input"] = "plumed.dat"
    step["command"] = "grompp"
    with pytest.raises(ValueError, match="only valid for mdrun"):
        normalize(body)
...
def test_explicit_file_expansion_retains_native_parts(tmp_path):
    ...
    assert expand_args(
        ["-f", {"files": "md.part*.xtc"}, "-o", "combined.xtc"], tmp_path
    ) == [
        "-f",
        "md.part0001.xtc",
        "md.part0002.xtc",
...
def test_peer_transport_failure_releases_engine_without_the_handoff_timeout(
    ...
    atomic_json(
        worker.meta / "transport-error.json", {"status": "failed", "phase": phase}
    )
    monkeypatch.setattr(
        "fs2_gromacs.worker.time.sleep",
        lambda _: (_ for _ in ()).throw(
            AssertionError("should not wait after peer failure")
        ),
    )
    if phase == "publish":
        ...
        assert (
            result["error"] == "durable checkpoint transport failed; see operation logs"
        )


@pytest.mark.parametrize("phase", ["restore", "publish"])
def test_pod_termination_does_not_wait_for_the_terminated_companion(
    tmp_path, monkeypatch, phase
):
    ...
    monkeypatch.setattr(
        "fs2_gromacs.worker.time.sleep",
        lambda _: (_ for _ in ()).throw(
            AssertionError("must release within Pod grace")
        ),
    )
    if phase == "publish":
