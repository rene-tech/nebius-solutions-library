...
from cluster import KUBE, NS, TASK, create, get, owned
...
def main():
    ...
    parser.add_argument("--resume-captured", action="store_true", help="Retry capacity admission after donor deletion; verify and reuse the exact preserved closed workspace")
    args = parser.parse_args()
    if args.pod == args.new_pod:
        raise ValueError("replacement must have a distinct Pod identity")
    if args.resume_captured:
        old = json.loads((args.output / "original-pod.json").read_text())
        if old["metadata"]["name"] != args.pod or old["metadata"].get("labels", {}).get("scientific-ai.nebius.com/task") != TASK:
            raise ValueError("captured original Pod identity is outside this task")
        if any(pod["metadata"]["name"] in {args.pod, args.new_pod} for pod in get(*NS, "get", "pods")["items"]):
            raise ValueError("capacity retry must not mutate an existing original or replacement Pod")
        if (args.output / "fresh-pod-recovery.json").exists():
            raise ValueError("do not overwrite completed recovery evidence")
    else:
        old = owned(args.pod)
    ...
    if not args.resume_captured:
        ...
        with (args.output / "interrupt-client.log").open("x") as log:
    ...
    for name in ("input.tar.gz", "request.json", "fixture-manifest.json"):
        if digest(donor / "restored" / name) != digest(args.fixture / name):
            raise ValueError("captured immutable fixture differs from requested recovery")
    ...
    receipt["replacement_capacity_admission_retried"] = args.resume_captured
