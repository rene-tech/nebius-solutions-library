# Complexa, BoltzGen, and mosaic runtime images
...
- The mosaic source archive itself contains three ProteinMPNN `.pt` files. The
  Dockerfile removes them before installing the package, so component models
  must be staged externally like every other mosaic objective dependency.
...
dependency locks include artifact hashes and pin all mosaic Git dependencies to
commits at or before the exact mosaic source commit. Builder, runtime, and `uv`
...
mosaic keeps the upstream-pinned CPU PyTorch `2.7.1` used for checkpoint
conversion. GPU execution is JAX `0.10.2` with its CUDA 12 plugin on an official
CUDA 12.9 runtime.
...
Repeat for `boltzgen` and `mosaic`. `publish` re-inspects the exact destination,
...
Mosaic follow-up moved to the dedicated
`fs2-cancer-images-mosaic-runtime-qualification-r20260903` task. The exact v2
image, H100 execution, adapter argv, external artifacts, source findings, and
semantic blocker are preserved in `evidence/mosaic-v2-handoff.md`; this branch
does not claim Mosaic model readiness.
