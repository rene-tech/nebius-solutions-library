...
REQUIRED = {
    ...
    "mosaic": {"mosaic/boltz/boltz2_conf.ckpt", "mosaic/boltz/mols.tar", "mosaic/proteinmpnn/v_48_020.pt"},
}
...
def _mosaic(output: Path, expected_digest: str) -> dict[str, Any]:
    ...
    required = {"shard-000", "aggregate", "candidate-000-metrics", "candidate-000-structure"}
    if set(entries) != required:
        raise SystemExit(f"mosaic manifest entries differ from one-shard contract: {sorted(entries)}")
    for entry in entries.values():
        pointer = entry["artifact"]
        path = Path(index[pointer["artifact_id"]])
        if path.stat().st_size != pointer["size_bytes"] or _sha256(path) != pointer["sha256"]:
            raise SystemExit(f"mosaic result artifact differs from manifest: {path}")
    aggregate = json.loads(Path(index[entries["aggregate"]["artifact"]["artifact_id"]]).read_text())
    if aggregate["runtime_image_digest"] != expected_digest or aggregate["succeeded_shards"] != 1 or not aggregate["atomic_commit"]:
        raise SystemExit("mosaic aggregate is not bound to the admitted image and complete shard set")
    metrics = json.loads(Path(index[entries["candidate-000-metrics"]["artifact"]["artifact_id"]]).read_text())
    if not 0 <= metrics["iptm"] <= 1 or not 0 <= metrics["mean_plddt"] <= 1 or len(metrics["sequence"]) != 40:
        raise SystemExit("mosaic candidate metrics violate semantic bounds")
    structure = _structure(Path(index[entries["candidate-000-structure"]["artifact"]["artifact_id"]]))
    structure.update({"iptm": metrics["iptm"], "mean_plddt": metrics["mean_plddt"], "objective": metrics["objective"], "sequence": metrics["sequence"]})
    return structure
...
def main() -> None:
    ...
    if args.model == "mosaic":
        if not args.runtime_image_digest:
            raise SystemExit("mosaic validation requires --runtime-image-digest")
        semantic = _mosaic(args.output, args.runtime_image_digest)
    else:
