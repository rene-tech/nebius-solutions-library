...
class RuntimeImageContractTest(unittest.TestCase):
    ...
    def test_exact_sources_targets_and_base_digests(self) -> None:
        ...
        self.assertEqual(2, len(self.catalog["images"]))
    ...
    def test_complexa_runtime_has_ptxas(self) -> None:
        image = next(item for item in self.catalog["images"] if item["id"] == "proteina-complexa")
        dockerfile = (ROOT / image["dockerfile"]).read_text(encoding="utf-8")
        self.assertIn("cuda-nvcc-12-6=12.6.85-1", dockerfile)
        self.assertIn("/usr/local/cuda-12.6/bin/ptxas", dockerfile)
        self.assertIn("CUDA_HOME=/usr/local/cuda-12.6", dockerfile)
