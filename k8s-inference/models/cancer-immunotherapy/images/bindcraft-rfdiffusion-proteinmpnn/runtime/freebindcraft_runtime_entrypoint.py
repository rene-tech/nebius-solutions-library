#!/usr/bin/env python3
"""Execute and aggregate deterministic PyRosetta-free BindCraft shards."""
...
BACKEND_ID = "freebindcraft-v1-0-5"
...
def _verify_open_lane() -> None:
    if importlib.util.find_spec("pyrosetta") is not None:
        raise ContractError("PyRosetta is forbidden for the open backend")
    if importlib.util.find_spec("freesasa") is None:
        raise ContractError("the pinned FreeSASA scorer is unavailable")


def _target_artifact(request: dict[str, Any], manifest: dict[str, Any]) -> Path:
    ...
    if not isinstance(entries, list) or len(entries) != 1 or entries[0].get("name") != "target_structure":
        raise ContractError("open BindCraft requires exactly one target_structure")
...
def _settings(request: dict[str, Any], target: Path, output: Path, template: Path) -> tuple[Path, Path]:
    ...
    target_settings = {
        ...
        "binder_name": f"fs2_open_s{parameters['_shard_index']:03d}",
...
def _run_upstream(
    ...
    original_package_check = functions.check_n_trajectories
    original_module_check = generic_utils.check_n_trajectories
    first = True
    attempts_started = 0
    ...
    def bounded_check(design_paths: dict[str, str], settings_value: dict[str, Any]) -> bool:
        nonlocal attempts_started
        if original_module_check(design_paths, settings_value):
            return True
        if attempts_started >= max_trajectories:
            print(f"FS2 hard trajectory bound of {max_trajectories} reached, stopping execution...")
            return True
        attempts_started += 1
        return False

    np.random.randint = deterministic_first_randint
    functions.check_n_trajectories = bounded_check
    generic_utils.check_n_trajectories = bounded_check
    old_argv = sys.argv
    try:
        sys.argv = [
            "bindcraft.py", "--settings", str(settings), "--filters", str(filters),
            "--advanced", str(advanced), "--no-pyrosetta", "--no-plots", "--no-animations",
        ]
        runpy.run_path("/opt/freebindcraft/bindcraft.py", run_name="__main__")
    finally:
        np.random.randint = original_randint
        functions.check_n_trajectories = original_package_check
        generic_utils.check_n_trajectories = original_module_check
        sys.argv = old_argv
...
def _openmm_energy(path: Path) -> float:
    import openmm
    ...
    simulation = app.Simulation(
        pdb.topology, system, integrator, platform, {"CudaPrecision": "mixed"},
    )
...
def run_trajectory(args: argparse.Namespace) -> None:
    if args.backend_id != BACKEND_ID or not args.no_pyrosetta or not args.pyrosetta_forbidden:
        raise ContractError("open backend identity and both PyRosetta-forbidden flags are mandatory")
    _verify_open_lane()
    ...
    target = _target_artifact(request, manifest)
    ...
    _run_upstream(
        ...
        advanced,
        args.seed,
        request["parameters"]["max_trajectories_per_shard"],
    )

    upstream = output / "upstream"
    final_csv = upstream / "final_design_stats.csv"
    if not final_csv.is_file():
        raise ContractError("upstream FreeBindCraft did not create final design statistics")
    ...
    from functions.pr_alternative_utils import pr_alternative_score_interface
    ...
    for candidate_index, row in enumerate(rows):
        candidate_id = f"open-s{args.shard_index:03d}-c{candidate_index:03d}"
        ...
        interface, _, _ = pr_alternative_score_interface(
            str(source_pdb), binder_chain="B", sasa_engine="freesasa",
        )
        metrics = {
            ...
            "openmm_energy_kj_mol": _openmm_energy(source_pdb),
            "buried_sasa_a2": float(interface["interface_dSASA"]),
            "shape_complementarity": float(interface["interface_sc"]),
        }
        ...
        metrics_id = f"artifact.bindcraft.open.s{args.shard_index:03d}.c{candidate_index:03d}.metrics"
        structure_id = f"artifact.bindcraft.open.s{args.shard_index:03d}.c{candidate_index:03d}.pdb"
        entries.extend([
            {"name": f"candidate-{candidate_index:03d}-metrics", "semantic_type": "bindcraft-open-design-metrics-json/v1", "artifact": _artifact(metrics_id, metrics_path, "application/json")},
    ...
    shard_id = f"artifact.bindcraft.open.shard.{args.shard_index:03d}"
    (output / "shard-output.json").write_text(json.dumps({
        "schema": "fs2-serve.nebius.ai/bindcraft-open-shard-output/v1",
        "shard": {"name": f"shard-{args.shard_index:03d}", "semantic_type": "bindcraft-open-shard-result-json/v1", "artifact": _artifact(shard_id, shard_path, "application/json")},
        "candidates": entries,
...
def aggregate(args: argparse.Namespace) -> None:
    ...
    aggregate_id = "artifact.bindcraft.open.aggregate"
    entries.append({"name": "aggregate", "semantic_type": "bindcraft-open-aggregate-json/v1", "artifact": _artifact(aggregate_id, aggregate_path, "application/json")})
    paths[aggregate_id] = str(aggregate_path.resolve())
    staging = Path(args.staging_manifest)
    staging.parent.mkdir(parents=True, exist_ok=True)
    staging.write_text(json.dumps({
        "schema": ARTIFACT_MANIFEST_SCHEMA,
        "manifest_id": "manifest.bindcraft.open.output",
        "entries": entries,
    }, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--no-pyrosetta", action="store_true")
    run.add_argument("--pyrosetta-forbidden", action="store_true")
...
if __name__ == "__main__":
    try:
        ...
        print(json.dumps({"event": "freebindcraft_batch_rejected", "reason": str(exc)}), file=sys.stderr)
