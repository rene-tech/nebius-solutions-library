...
All four image bindings remain `build-only-not-semantic-qualified`. Their candidate
...
the OpenFold r4 image is not runnable as runtime UID 10001 and must be replaced by an
immutable, qualified r5 digest before promotion. The canonical machine-readable
