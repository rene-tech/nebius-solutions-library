...
def _job(*, image: str, name: str, command: list[str], gpu: bool, labels: dict[str, str], annotations: dict[str, str], config_name: str, access_receipt_digest: str) -> dict[str, Any]:
    ...
    pod_spec = {
        ...
        "securityContext": {"runAsNonRoot": True, "runAsUser": 10001, "runAsGroup": 10001, "supplementalGroups": [65532], "seccompProfile": {"type": "RuntimeDefault"}},
        "imagePullSecrets": [{"name": "fs2-runtime-registry"}],
        "initContainers": [{
            "name": "academic-access-gate", "image": image, "imagePullPolicy": "IfNotPresent",
            "command": [
                "/opt/fs2/bin/verify-academic-access", "--profile", "academic-noncommercial",
            ...
            "volumeMounts": [{"name": "access-receipt", "mountPath": "/var/run/fs2-access", "readOnly": True}, {"name": "academic-assets", "mountPath": "/opt/fs2/academic/pyrosetta-bindcraft/site-packages", "subPath": "pyrosetta-bindcraft/site-packages", "readOnly": True}],
        }],
        "containers": [{
            "name": "batch", "image": image, "imagePullPolicy": "IfNotPresent",
            "command": command, "resources": resources, "securityContext": security,
            "env": [{"name": "PYTHONPATH", "value": "/opt/fs2/academic/pyrosetta-bindcraft/site-packages:/opt/fs2"}],
            "volumeMounts": [
                {"name": "request", "mountPath": "/var/run/fs2", "readOnly": True},
                {"name": "access-receipt", "mountPath": "/var/run/fs2-access", "readOnly": True},
                {"name": "workspace", "mountPath": "/workspace"},
                {"name": "tmp", "mountPath": "/tmp"},
                {"name": "academic-assets", "mountPath": "/opt/fs2/academic/pyrosetta-bindcraft/site-packages", "subPath": "pyrosetta-bindcraft/site-packages", "readOnly": True},
            ],
        }],
        "volumes": [
            ...
            {"name": "academic-assets", "artifactMaterialization": {"artifact_id": "bindcraft-pyrosetta-installed-tree", "subpath": "pyrosetta-bindcraft/site-packages", "digest": access_receipt_digest, "access": "restricted-readonly"}},
        ],
    }
    return {
