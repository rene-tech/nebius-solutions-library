...
def test_receipt_projects_to_controller_wire_shape_without_inventing_qualification() -> None:
    ...
    assert evidence["compatibilityTupleDigest"] == f"sha256:{receipt['compatibility_tuple_digest']}"
    assert evidence["compatibilityTupleComplete"] is True
    assert evidence["measurementBasis"] == "CapacityAvailableToSemanticReady"
    assert evidence["templateDigest"] == receipt["compatibility_tuple"]["runtime_template_digest"]
...
def test_incomplete_tuple_is_preserved_as_non_qualifying_controller_evidence() -> None:
    receipt = BENCHMARK.build_receipt(
        [attempt(index, model_start=90, tuple_overrides={"driver_version": None}) for index in range(1, 21)],
        generated_at="2026-09-02T13:00:00Z",
    )
