...
class RegistryProbeTests(unittest.TestCase):
    ...
    def test_registry_service_is_taken_from_verified_challenge(self):
        ...
        def handler(request):
            ...
            return httpx.Response(401, headers={"WWW-Authenticate": 'Bearer realm="https://nvcr.io/proxy_auth",service="registry"'})
    ...
    def test_rejects_external_realm_without_sending_credentials(self):
        ...
        def handler(request):
            requests.append(request)
            return httpx.Response(401, headers={"WWW-Authenticate": 'Bearer realm="https://example.invalid/proxy_auth",service="registry"'})
    ...
    def test_exception_details_are_not_printed(self):
        output = io.StringIO()
        with mock.patch.dict(probe.os.environ, {"NGC_API_KEY": "synthetic-key"}), mock.patch.object(probe, "probe", side_effect=ValueError("synthetic-key private-response")), contextlib.redirect_stdout(output):
