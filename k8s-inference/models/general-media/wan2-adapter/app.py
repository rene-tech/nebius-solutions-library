...
MODEL_REVISION = "nim-1.0.0@sha256:05c1d390af4eec607b654172fa889ae8cef2b2c238e84516514e61e5ba52e63b"
...
class GenerateRequest(BaseModel):
    ...
    negative_prompt: Annotated[str | None, Field(min_length=1, max_length=4096)] = None
    ...
    input_reference: Annotated[str | None, Field(max_length=MAX_IMAGE_DATA_URL_CHARS)] = None

    @model_validator(mode="after")
    def validate_variant(self) -> "GenerateRequest":
        if VARIANT == "i2v" and self.input_reference is None:
            raise ValueError("input_reference is required by the image-to-video deployment")
        if VARIANT == "t2v" and self.input_reference is not None:
            raise ValueError("input_reference is not accepted by the text-to-video deployment")
...
def _verified_mp4(raw: bytes, request: GenerateRequest) -> tuple[bytes, dict[str, object]]:
    if not raw or len(raw) > MAX_VIDEO_BYTES:
        raise HTTPException(status_code=502, detail="NIM returned an empty or oversized video")
    with tempfile.TemporaryDirectory(prefix="fs2-wan2-output-") as directory:
        ...
        try:
            ...
            raise HTTPException(status_code=502, detail="NIM output is not a decodable MP4") from error
    streams = [stream for stream in probe.get("streams", []) if stream.get("codec_type") == "video"]
    if len(streams) != 1 or "mp4" not in str(probe.get("format", {}).get("format_name", "")):
        raise HTTPException(status_code=502, detail="NIM output does not contain exactly one MP4 video stream")
    video = streams[0]
    ...
    if (video.get("width"), video.get("height")) != (expected_width, expected_height):
        raise HTTPException(status_code=502, detail="NIM output dimensions differ from the request")
    try:
        duration = float(probe["format"]["duration"])
    except (KeyError, TypeError, ValueError) as error:
        raise HTTPException(status_code=502, detail="NIM output has invalid duration metadata") from error
    if not 0 < duration <= request.seconds + 0.25:
        raise HTTPException(status_code=502, detail="NIM output duration is outside the request bound")
    return raw, {
...
def _nim_error(response: httpx.Response) -> HTTPException:
    if response.status_code == 422:
        return HTTPException(status_code=422, detail="Wan2.2 rejected the request or its safety filter blocked the output")
    if response.status_code == 429:
        return HTTPException(status_code=429, detail="Wan2.2 is busy; retry this operation later")
    if response.status_code in {502, 503, 504}:
        return HTTPException(status_code=503, detail="Wan2.2 is temporarily unavailable")
...
async def lifespan(_: FastAPI):
    ...
    nim_client = httpx.AsyncClient(timeout=httpx.Timeout(620, connect=10), trust_env=False)
    yield
...
async def ready() -> dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Wan2.2 NIM is not ready") from error
    if response.status_code != 200:
        raise HTTPException(status_code=503, detail="Wan2.2 NIM is not ready")
    return {"status": "ready", "model": MODEL_ID, "revision": MODEL_REVISION, "variant": VARIANT}
...
async def generate(request: GenerateRequest) -> Response:
    ...
    async with generation_lock:
        try:
            upstream = await nim_client.post(f"{UPSTREAM}/v1/videos/generations", json=payload)
        except httpx.TimeoutException as error:
            raise HTTPException(status_code=504, detail="Wan2.2 generation exceeded the bounded runtime timeout") from error
        except httpx.HTTPError as error:
            raise HTTPException(status_code=503, detail="Wan2.2 transport failed") from error
    if upstream.status_code != 200:
        raise _nim_error(upstream)
    try:
        document = upstream.json()
        encoded = document["data"]["b64_json"]
        if not isinstance(encoded, str) or len(encoded) > ((MAX_VIDEO_BYTES + 2) // 3) * 4 + 4:
        ...
        raise HTTPException(status_code=502, detail="Wan2.2 returned an invalid response envelope") from error
