...
def marker_path(root: Path, challenge: str) -> Path:
    return root / f".fs2-durability-{hashlib.sha256(challenge.encode()).hexdigest()[:24]}"
...
def proof(args: argparse.Namespace) -> dict[str, object]:
    ...
    target = marker_path(root, args.challenge)
    if args.mode == "write":
    ...
    result: dict[str, object] = {
        "schema": "fs2-serve.nebius.ai/checkpoint-durability-proof/v1",
        "mode": args.mode,
        "pvc": identity,
        "challenge": args.challenge,
