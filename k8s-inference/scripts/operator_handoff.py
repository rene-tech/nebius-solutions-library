...
def host_cidrs(values: Sequence[str]) -> list[str]:
    ...
    for value in values:
        ...
        if network.prefixlen != network.max_prefixlen:
            raise HandoffError("approved egress entries must be exact IPv4 /32 or IPv6 /128 hosts")
...
def run(
    ...
    *,
    ...
    try:
        ...
        raise HandoffError(f"handoff command failed: {arguments[0]} {arguments[1]}") from exc
...
def load_receipt(directory: Path) -> tuple[Path, dict[str, Any]]:
    ...
    if value.get("schema") != "fs2-serve.nebius.ai/operator-handoff/v1":
...
def issue(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if expiry <= now or expiry > now + timedelta(days=366):
        raise HandoffError("handoff expiry must be future and no more than one year away")
    private_key, public_key = directory / "private-key.pem", directory / "public-key.pem"
    if any(path.exists() for path in (private_key, public_key, directory / "receipt.json")):
        raise HandoffError("handoff directory already contains key or receipt material")
    run([args.openssl, "genpkey", "-algorithm", "RSA", "-pkeyopt", "rsa_keygen_bits:4096", "-out", str(private_key)])
    private_key.chmod(0o600)
    run([args.openssl, "pkey", "-in", str(private_key), "-pubout", "-out", str(public_key)])
    ...
    try:
        result = run(
            [args.nebius, "iam", "auth-public-key", "create", "--profile", args.admin_profile, "--file", str(request_path), "--format", "json"],
            capture=True,
        )
    ...
    return {"status": "issued", "public_key_id": key_id, "expires_at": receipt["expires_at"]}
...
def acknowledge(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {"status": "delivery-acknowledged", "public_key_id": receipt["public_key_id"]}
...
def verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    run([
        args.nebius, "profile", "create", profile, "--config", str(config),
        "--service-account-id", receipt["service_account_id"], "--public-key-id", receipt["public_key_id"],
        "--private-key-file-path", str(private_key), "--parent-id", receipt["project_id"],
    ])
    config.chmod(0o600)
    run([
        args.nebius, "mk8s", "v1", "cluster", "get-credentials", "--config", str(config), "--profile", profile,
        "--id", receipt["cluster_id"], "--kubeconfig", str(kubeconfig),
    ])
    kubeconfig.chmod(0o600)
    run([args.kubectl, "--kubeconfig", str(kubeconfig), "get", "namespaces", "-o", "name"], capture=True)
    ...
    cluster = json.loads(run([
        args.nebius, "mk8s", "v1", "cluster", "get", "--config", str(config), "--profile", profile,
        "--id", receipt["cluster_id"], "--format", "json",
    ], capture=True).stdout)
    observed = host_cidrs(cluster["spec"]["control_plane"]["endpoints"]["public_endpoint"]["allowed_cidrs"])
    if observed != approved:
        raise HandoffError("live control-plane CIDRs differ from the approved egress set")
    ...
    private_json(receipt_path, receipt)
    return {"status": "verified", "public_key_id": receipt["public_key_id"], "expires_at": receipt["expires_at"]}
...
def revoke_old(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if receipt["verification"] is None or receipt["delivery"] is None:
        raise HandoffError("verified delivery is required before revoking the old handoff")
    if args.old_public_key_id == receipt["public_key_id"]:
        raise HandoffError("refusing to revoke the newly verified handoff key")
    run([args.nebius, "iam", "auth-public-key", "delete", "--profile", args.admin_profile, "--id", args.old_public_key_id])
    receipt["revoked_old_key"] = {
        "public_key_id": args.old_public_key_id,
        "revoked_at": utc_now().isoformat().replace("+00:00", "Z"),
    }
    private_json(receipt_path, receipt)
    return {"status": "old-key-revoked", "public_key_id": args.old_public_key_id}
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    revoke_parser.add_argument("--old-public-key-id", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    handlers = {"issue": issue, "acknowledge-delivery": acknowledge, "verify": verify, "revoke-old": revoke_old}
