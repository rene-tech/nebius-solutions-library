...
(`/cache/protenix` in `fs2-models`). Each claim is same-namespace and
operator-owned, has no runtime-artifact identity, and uses exact image-contract
environment paths. The generated v3 map does not expose the claim root to the
model. It binds a safe subtree whose path contains the model, stage, variant,
immutable runtime-image SHA-256, stage artifact-set SHA-256, and reviewed CUDA
SM (`sm90`). Before the model starts, a controller-owned init command creates
only that subtree, writes an immutable identity marker, changes that directory
to UID/GID `10001`, and drops irreversibly to `10001:10001` to perform a real
create/fsync/read/remove probe. A later attempt validates the same marker and
repeats the probe without deleting warm cache entries. The init container sees
no model or database volumes, and the Pod does not use `fsGroup`, avoiding
recursive ownership changes to large read-only trees.

These bindings are auxiliary L1+ compiler/JIT optimizations, not L2. Their
numbered ceiling is `L1`, their current qualified level is `Off`, and routing
remains unqualified until first/warm H100 compile measurements. L2 requires an
actual GPU/process snapshot on shared filesystem or object storage; L3 requires
a local-disk snapshot, and L4 requires host-RAM residency.
