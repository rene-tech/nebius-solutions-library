...
# capacity.
#
# This example is policy only. It selects no models, leaves the scientific
# batch controller gate off, and therefore claims nothing about which model
# runs where: placing a model on an accelerator it has not been qualified for
# would be a false readiness claim, and no model in this repository's catalog
# is H100-qualified yet. It shows the queue topology, the weights, and the
# service classes; add qualified models and their lanes once their placement
# contract lists an H100 pool.
#
...
deployment = {
  ...
  scheduling = {
    ...
    local_queues = {
      # Weighted lanes with no model routes yet. A lane only carries model and
      # tenant routes once a qualified model exists for these pools; until
      # then each service class simply defaults to its lane.
      scientific-primary = {
