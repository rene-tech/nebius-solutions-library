...
CASES = [
    ...
    (
        "translation",
        "Translate the English noun cat into German. Reply with only Katze.",
        "Katze",
    ),
    (
        "sorting",
        "Sort the letters T G A C alphabetically. Reply with only ACGT, no spaces.",
        "ACGT",
    ),
]
...
def phase(path):
    return (
        json.loads(path.read_text()).get("phase", "baseline")
        if path.exists()
        else "baseline"
    )


def safe_exception(error, *, depth=0):
    """Retain nested types/status/codes, never SDK exception text or headers."""
    ...
    if isinstance(code, int):
        record["protocol_code"] = code
    children = getattr(error, "exceptions", None)
    if isinstance(children, tuple):
        record["exceptions"] = [
            safe_exception(child, depth=depth + 1) for child in children[:16]
        ]
        record["omitted_children"] = max(0, len(children) - 16)
    elif error.__cause__ is not None:
        record["cause"] = safe_exception(error.__cause__, depth=depth + 1)
    elif error.__context__ is not None:
        record["context"] = safe_exception(error.__context__, depth=depth + 1)
    return record


def safe_tool_error(result):
    """Allowlist machine fields; raw server text might contain private data."""
    payload = result.structured_content
    detail = payload.get("error") if isinstance(payload, dict) else None
    detail = detail if isinstance(detail, dict) else {}
    output = {"is_error": bool(result.is_error)}
    kind = detail.get("type")
    if (
        isinstance(kind, str)
        and len(kind) <= 64
        and all(c in "abcdefghijklmnopqrstuvwxyz0123456789_-" for c in kind)
    ):
        output["type"] = kind
    if isinstance(detail.get("retryable"), bool):
        output["retryable"] = detail["retryable"]
    try:
        output["request_id"] = str(UUID(str(detail["request_id"])))
...
async def sample(args, credentials, ordinal):
    ...
    key = "trial-remediation-" + uuid4().hex
    record["idempotency_key"] = key
    record["mcp_calls"] = []

    def mcp_payload(result, tool):
        record["mcp_calls"].append({"tool": tool, **safe_tool_error(result)})
        return _mcp_result(result)

    async def response_observer(response):
        record.setdefault("mcp_http_statuses", []).append(response.status_code)
    ...
    try:
        if surface == "http":
            async with httpx.AsyncClient(
                ...
                trust_env=False,
                headers={
                    "authorization": "Bearer " + credentials["inference_access_token"]
                },
            ) as client:
                ...
                operation_id = (
                    response.headers.get("x-fs2-operation-id") or current["id"]
                )
            ...
            async with httpx2.AsyncClient(
                ...
                headers={
                    "authorization": "Bearer " + credentials["mcp_inference_token"],
                    "origin": args.origin,
                },
                event_hooks={"response": [response_observer]},
            ) as http:
                async with Client(
                    ...
                    record.update(
                        operation_id=operation_id, mcp_protocol=client.protocol_version
                    )
                    while current.get("status") not in TERMINAL:
                        if time.monotonic() - started > 120:
                            raise TimeoutError("operation deadline")
        ...
        record.update(
            ...
            correctness_passed=content == expected,
            status="passed" if content == expected else "failed",
        ...
        record.update(
            ...
            failure_detail=safe_exception(error),
        )
    ...
    record.update(
        completed_at=utc(), client_seconds=round(time.monotonic() - started, 6)
    )
    return record
...
async def discovery(args, credentials):
    ...
    async with httpx.AsyncClient(
        base_url=args.origin, timeout=30, trust_env=False
    ) as client:
        for path, credential in (
            ("/readyz", "inference_access_token"),
            ("/v1/models", "inference_access_token"),
...
async def main():
    ...
    if args.resume_after:
        ...
        elapsed = (
            datetime.now(timezone.utc) - datetime.fromisoformat(prior["started_at"])
        ).total_seconds()
        ...
        json.dumps(
            {
                ...
                "resumes_after_ordinal": ordinal,
                "resume_receipt_sha256": hashlib.sha256(
                    args.resume_after.read_bytes()
                ).hexdigest()
                if args.resume_after
                else None,
                "public_health_path": "/readyz",
                "tls_verified": True,
    ...
    first = True
    with (
        (args.output / "interactive.jsonl").open("x") as results,
        (args.output / "discovery.jsonl").open("x") as checks,
    ):
        while not (args.output / "stop.json").exists():
            ...
            if first or ordinal % 4 == 0:
                ...
                first = False
            while (
                time.monotonic() - start < args.interval
                and not (args.output / "stop.json").exists()
            ):
                await asyncio.sleep(0.5)
    completion = {"status": "stopped", "samples": ordinal, "stopped_at": utc()}
    with (args.output / "sampler-completed.json").open("x") as completed:
