#!/usr/bin/env python3
"""Create, validate, capture, and remove the isolated GenMol snapshot donor."""
...
from validate_genmol import validate


HERE = Path(__file__).resolve().parent
TASK = "fs2-h100-fleet-bionemo-structure-r20260907"
MODEL_REVISION = "2acccbd6eee62f2a90334ccf14dc7d2b17ef9e80"
...
def main() -> None:
    ...
    parser.add_argument("action", choices=("create", "observe", "validate", "capture", "delete"))
    parser.add_argument("--kubeconfig", required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--source", type=Path)
    parser.add_argument("--pod", default="fs2-genmol-snapshot-donor-r01")
    parser.add_argument("--run", default="genmol-r01")
    parser.add_argument("--node", default="computeinstance-e00p3acr87k9k4mckj")
    parser.add_argument(
        "--source-configmap", default="fs2-fleet-snapshot-serving-workdir-v8"
    )
    ...
    if args.action == "create":
        ...
        config = argparse.Namespace(
            ...
            # This image intentionally has no uvloop package; Uvicorn already
            # selects the standard asyncio loop, so no compatibility shim is
            # needed. Normal and restore trials retain this exact variant.
            asyncio_loop=False,
            python="python3",
            run=args.run,
            fallback="fail",
        ...
        runtime = next(
            item for item in pod["spec"]["containers"] if item["name"] == "genmol"
        )
        network_path = next(item for item in runtime["env"] if item["name"] == "PATH")
        network_path["value"] = (
            "/tools/usr/sbin:/opt/conda/bin:/usr/local/nvidia/bin:"
    ...
    if args.action == "delete":
        ...
        save(
            "deleted.json",
            {"uid": pod["metadata"]["uid"], "completed_at": utc(), "result": result.stdout},
        )
        return
    if args.action == "validate":
        print(
            json.dumps(
                validate(args.kubeconfig, args.pod, args.output / "semantics")
            ),
            flush=True,
        )
        return
    if args.action == "observe":
        for _ in range(450):
            ...
            if alive.returncode:
                raise RuntimeError("snapshot donor worker exited before application readiness")
            probe = call(
                [
                    "exec",
                    args.pod,
                    ...
                    (
                        ...
                        "'http://127.0.0.1:8000/v1/health/ready',timeout=2).status)"
                    ),
                ],
                check=False,
            )
    ...
    result = call(
        ["exec", args.pod, "-c", "genmol", "--", "python3", "-c", code],
        timeout=900,
        check=False,
    )
