...
def main():
    ...
    if pod["metadata"]["labels"].get("workload.fs2.nebius/owner") != "nemotron-speech-20260916":
        raise ValueError("not a task-owned speech worker")
    raw = json.loads(subprocess.check_output(kubectl + ["-n", "fs2-system", "get", "secret", "fs2-serve-admin",
                                                       "-o", "json"]))
    token = base64.b64decode(raw["data"]["token"]).decode().strip()
    receipt = {"scope":"private resident worker HTTP/WebSocket; not public gateway acceptance",
               "started_at":datetime.now(UTC).isoformat(), "pod":args.pod, "pod_uid":pod["metadata"]["uid"],
               "image":pod["spec"]["containers"][0]["image"], "model":args.model, "measurements":[]}
    objects, s3 = [], None
    with httpx.Client(base_url=args.origin, headers={"origin":args.origin}, timeout=60, trust_env=False) as admin:
        try:
            if admin.post("/admin/api/v1/session", headers={"authorization":"Bearer " + token}).status_code != 200:
            ...
            second = (("de-herzrasen", "ready/de/hhu-herzrasen.wav", "de-DE") if "multilingual" in args.model else
                      ("en-02", "ready/en/day1_consultation02_conversation.wav", "en-US"))
            for case, relative, locale in [("en-01", "ready/en/day1_consultation01_conversation.wav", "en-US"), second]:
                ...
                payload = {"audio":{"url":url,"sha256":sha,"size_bytes":path.stat().st_size,"media_type":"audio/wav"},
                           "options":{"model":args.model,"language":locale,"output_granularity":"word"}}
                started = datetime.now(UTC).isoformat()
                process = subprocess.run(kubectl + ["-n","fs2-models","exec","-i",args.pod,"-c","speech","--",
                                                     "/opt/conda/bin/python","-c",DRIVER],
                                         input=json.dumps(payload).encode(), capture_output=True, timeout=900)
                # Do not dump stderr/argv or signed input on failure.
                if process.returncode:
                    receipt["error"] = {"case":case,"code":"private_worker_probe_failed","returncode":process.returncode}
                    raise RuntimeError("private_worker_probe_failed; inspect task-owned worker logs")
                ...
                for row in rows:
                    receipt["measurements"].append({"case":case,"input_sha256":sha,"started_at":started,**row})
                args.output.write_text(json.dumps(receipt, ensure_ascii=False, indent=2) + "\n")
                print(json.dumps({"case":case,"model":args.model,"http_and_live":"passed"}), flush=True)
